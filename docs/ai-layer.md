...
>
> 当前实现边界（V2.41-V2.94 implementation line）：
>
...
> - V2.44-V2.67 已落地：task readiness、routing confidence、benchmark/regression、trace import、routing accuracy、cross-agent readiness、stale/drift、knowledge index、similar grouping、capability taxonomy、workspace readiness、remediation planning/drafts/impact/batch/history、app-local prompt run history、agent session skill review、local skill map、provider observability、task-first cockpit、skill lifecycle timeline，以及 guided cleanup flow；这些能力默认仍是 user-triggered、read-only、local evidence-first。
...
> - V2.73-V2.96 已完成/实现 timeout recovery、launch/window targeting、task input resilience、progressive feedback、validation workbench、protocol/gate parity、privacy/localization、Detail density、Swift IPC cancellation、test isolation、continued module splitting、Swift Detail section split、Rust RPC domain split、Rust helper/test split、module-size gate、Agent Copilot first pass 的 Lineup / Agent Profile / local session preview / MCP server preview、V2.88 per-surface evidence closeout、V2.89 visual-only brand asset refresh、V2.90 compatibility-first identifier migration、V2.91 local model-task matching history、V2.92 Codex expanded roots、V2.93 opencode configured local roots、V2.94 Pi install/compat writes、V2.95 Hermes native-root install，以及 V2.96 OpenClaw native/workspace install；这些切片不扩展 provider/default-write/script/credential/cloud/telemetry 边界。V2.87 于 2026-06-17 解锁后通过 `pnpm check:macos`，V2.88 补齐 Lineup / Agent Profile / Local Session Preview / MCP Preview 的 Computer Use app-window evidence，V2.89 刷新 `AppIcon.svg` / `AppIcon.icns`，V2.90 迁移到 `dist/AgentCopilot.app` / `dev.agent-copilot.native` 并保留 legacy compatibility，V2.91 通过 `model-task-matches.json` 与 `model_task_history_rows` 增加本地只读观测证据，V2.92 为 Codex 增加 read-only `$CODEX_HOME/skills`、plugin marketplace、`/etc/codex/skills` 和 project `.codex/config.toml` diagnostics，同时保持 native `.agents/skills` write allowlist；V2.93 为 opencode 增加 JSON/JSONC `skills.paths` configured local root scanning，同时保持 `skills.urls` metadata-only/no-fetch、configured roots read-only、install native-root-only；V2.94 为 Pi 增加 `.agents/skills` compatibility scanning/toggles 与 native-root installs，同时保持 package install/remove 和 `.agents` direct installs blocked；V2.95 为 Hermes 增加 native `~/.hermes/skills` installs；V2.96 为 OpenClaw 增加 native `~/.openclaw/skills` 与 confirmed workspace `<workspace>/skills` installs，同时保持 `.agents` direct installs、config toggles、`skills.entries` writes 和 ClawHub/Git/network operations blocked。
>
> V2.45（已完成）：
>
...
| 真实 agent 会话中的 skill 使用审查 | 本地证据 | 用户导入 trace、粘贴 transcript，或未来显式选择本地 agent 会话后主动生成；V2.62 不发送 provider request |
| 本地 skill map / 关系图 | 本地证据 | 用户主动触发 `knowledge.buildLocalSkillMap`；V2.63 只派生 existing catalog/knowledge/similar/taxonomy/conflict/task/risk evidence，不创建新的 source of truth |
| provider 调用可观测性 | 本地 provider/prompt metadata | 用户主动触发 `llm.providerObservability`；V2.64 只派生 app-local redacted prompt run metadata 与 minimal provider call metadata，不发送 provider request |
...
## 1.2 V2.41-V2.94 AI-native / validation / modularity 能力线
...
| V2.62（实现） | Agent Session Skill Review（`session.reviewAgentSkillUse` / `session.listSkillReviews` / `session.deleteSkillReview`） | V2.48 imported trace、用户粘贴/导入的 agent transcript、未来显式选择的本地 agent session；审查 skill 发现/选择/使用、hit/miss/wrong-pick/ambiguous/unknown、detected vs expected skills、重复/相似 skill 干扰、safe next steps 与 evidence refs；app-local `agent-session-reviews.json` redacted metadata only，app AI prompt runs 仅作辅助证据 |
| V2.63（实现） | Local Skill Map（`knowledge.buildLocalSkillMap`） | existing catalog evidence、knowledge tags、similar groups、capability taxonomy、conflicts、cross-agent analysis、task coverage、readiness/routing/session-review context、stale/drift 与 risk evidence；user-triggered deterministic read-only map；no new source of truth、no default persistence/provider/write/script/snapshot/triage/cloud/telemetry |
| V2.64（completed） | Provider observability / `llm.providerObservability` | V2.41-V2.42 minimal call metadata、V2.61 prompt run metadata；summary、call/history rows、provider/model/destination grouping rows、status rows、budget usage hints、retention recommendations、evidence refs、prompt metadata、safety flags；默认不持久化 raw prompt/raw response JSON/API keys/credentials/raw traces/unredacted paths，不发 provider request |
| V2.65（completed） | Task-first Cockpit / `task.buildCockpit` | task readiness、routing confidence、benchmark/regression、trace/session review、provider run context、remediation next steps；按用户任务聚合 local evidence，不创建 hidden task state |
| V2.66（completed） | Skill Lifecycle Timeline / `skill.lifecycleTimeline` | existing catalog evidence、scan/provenance/fingerprint state、stale/drift、finding/triage/remediation history、prompt run metadata、provider observability metadata、session review outcomes；生成 per-skill/per-agent/per-workspace lifecycle rows；默认不持久化 new raw lifecycle artifacts、不发 provider request、不写 skill/config/triage/snapshot |
| V2.67（completed） | Guided Cleanup Flow / `cleanup.planGuidedFlow` / `cleanup.recordGuidedStep` | `cleanup.planGuidedFlow` user-triggered deterministic/read-only over findings、cleanup queue、similar groups、stale/drift、readiness/routing/task cockpit、lifecycle timeline、remediation plan/drafts/impact/batch review、adapter diagnostics、source provenance；`cleanup.recordGuidedStep` only app-local redacted `guided-cleanup-steps.json` metadata；所有 enable/disable/edit/remediation 写入继续走 existing preview-first / explicit-confirm safe paths |
...
| V2.87（completed） | Agent Copilot first pass | Lineup default surface, Agent Profile, sorted read-only decision queue, default-off `session.previewLocalSessions`, default-off `evidence.previewMcpServers`, and then-current 90-method protocol parity |
...
| V2.90（completed） | Identifier migration | Packaged app identity and default app-data id migrated to `AgentCopilot` / `dev.agent-copilot.native`; legacy app-data compatibility retained without Keychain secret copying or AX/env/module rename |
| V2.91（completed） | Model-task matching history | App-local redacted `model-task-matches.json`, `llm.listModelTaskMatches` / `llm.recordModelTaskMatch` / `llm.deleteModelTaskMatch`, and Provider Observability `model_task_history_rows`; no provider request, credential read, raw prompt/response/trace persistence, skill/config write, script execution, cloud sync, or telemetry |
| V2.92（completed） | Codex expanded root diagnostics | Read-only `$CODEX_HOME/skills`, local plugin marketplace roots, `/etc/codex/skills`, project `.codex/config.toml` diagnostics, and native `.agents/skills` write allowlist; no provider request, credential read, raw prompt/response/trace persistence, plugin hook, network fetch, script execution, cloud sync, telemetry, or expanded config write |
| V2.93（completed） | opencode configured local root diagnostics | Read-only JSON/JSONC `skills.paths` local directory scanning with `RootSource::Configured`, canonicalization/dedupe, project-boundary checks, and configured provenance labels; `skills.urls` metadata-only/no-fetch; no provider request, credential read, raw prompt/response/trace persistence, script execution, cloud sync, telemetry, URL fetch, or configured-root write/install target |
| V2.94（completed） | Pi install and compatibility-root writes | `RootSource::Compatibility` for Pi `.agents/skills`, guarded native/compat toggles through Pi settings with `project.trusted` checks, and native-root direct installs to `~/.pi/agent/skills` / project `.pi/skills`; package install/remove and `.agents` direct installs remain blocked; no provider request, credential read, raw prompt/response/trace persistence, script execution, cloud sync, or telemetry |
...
V2.64 Provider Observability completed：`llm.providerObservability` 在用户显式触发后，只从 app-local V2.61 prompt run metadata 与 existing minimal provider call metadata 派生 provider 使用视图。输出包括 summary、call/history rows、provider/model/destination grouping rows、status rows、budget usage hints、retention recommendations、evidence refs、prompt metadata 与 safety flags。该路径 deterministic/read-only，不持久化 raw prompt、raw provider response JSON、API key、credentials、raw trace、secrets 或 unredacted local paths；不写 skill/config、不改 triage、不创建/回滚 snapshot、不执行脚本、不默认发 provider/network request、不云同步、不发 telemetry。V2.65 Task-first Cockpit completed：`task.buildCockpit` 只聚合 existing local task/readiness/routing/session/provider/remediation evidence 与 redacted metadata，不创建 hidden task state source，不发 provider request，不新增 write/execute/snapshot/triage/credential/cloud/telemetry 路径。V2.66 Skill Lifecycle Timeline completed：`skill.lifecycleTimeline` 只从 existing catalog evidence、scan/provenance/fingerprint state、stale/drift、finding/triage/remediation history、prompt run metadata、provider observability metadata 与 session review outcomes 派生 per-skill/per-agent/per-workspace lifecycle rows；不默认持久化 new raw lifecycle artifacts，不写 skill/config、不改 triage、不创建/回滚 snapshot、不执行脚本、不默认发 provider/network request、不读 credentials、不持久化 raw prompt/raw response/raw trace/secrets/unredacted local paths、不云同步、不发 telemetry。V2.67 Guided Cleanup Flow completed：`cleanup.planGuidedFlow` 只把 existing local evidence 组织为可复查 cleanup flow，`cleanup.recordGuidedStep` 仅可写 app-local redacted step metadata；不得写 skill/config、改 triage、创建/回滚 snapshot、执行脚本、读取 credentials、默认发送 provider/network request、持久化 raw prompt/raw response/raw trace/secrets/unredacted paths、cloud sync 或 telemetry。V2.71 Guided Cleanup safe-action links completed：safe links 只连接到 existing safe surfaces，不新增 apply/write/provider/execute 语义。V2.72 Validation harness hardening completed：只加固本地验证/截图/Computer Use blocker 分类，不新增 AI/provider/runtime semantics。V2.73 Task/remediation timeout recovery completed：只增加 bounded local aggregation、fallback/partial diagnostics、cancel/retry UI 与 stale completion protection，不新增 provider/write/execute/credential/cloud/telemetry 语义。Export/cleanup 在本切片中仅作为 recommendations，真实写入仍走 existing preview-first / explicit-confirm safe methods。
