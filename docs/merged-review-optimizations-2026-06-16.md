...
- 已完成项和后续维护边界同步到 `docs/development-tasks.md`；release-risk 摘要同步到 `CHANGELOG.md`。
...
- P1 结构项已落地：service helper/test `include!` 改真实模块，`commands` 拆出 `analysis.rs` / `script_execution.rs` / `tests.rs` 并降到 `< 5k`，catalog 拆成 `schema.rs` / `mapping.rs` / `queries.rs` / `refresh.rs`，adapter 共享 helper 拆到 `shared.rs`，Swift `ServiceClient` transport 与 RPC domain extension 拆文件，`SkillStore` derived state、navigation actions、workflow selectors 拆文件。
- P2 门禁项已落地：新增 `verify:js-syntax`、`verify:rust-docs`、`verify:benchmark-trends`，CI 新增 `cargo audit` 与 Rust API docs job，V2.73-V2.86 docs verifier 合并到 `scripts/verify-version-validation-docs.mjs`，task readiness / routing / knowledge search benchmark 均已有专项脚本和趋势记录。

该报告不再保留独立未完成任务。后续更深的 Swift domain-store 演进只在确有行为改动或状态所有权收益时进入 `docs/development-tasks.md`，不得为了形式拆分放宽 `@Published private(set)` 写权限。
...
Closeout：本轮不放宽 `@Published private(set)` 写权限，先将已稳定且只读或导航性质的边界拆出：`SkillStoreDerivedState.swift`、`SkillStoreNavigationActions.swift`、`SkillStoreWorkflowSelectors.swift`。`SkillStore.swift` 降到 `< 3k` 行，并由 Swift tests 与 `verify:module-size` 继续约束；更深 domain-store 只在后续有真实状态所有权收益时推进。
...
Closeout：`crates/catalog/src/lib.rs` 已拆为 `schema.rs`、`mapping.rs`、`queries.rs`、`refresh.rs`，保留 crate-root 公共类型 re-export 和 SQLite transaction 语义。`cargo test -p skills-copilot-catalog -- --nocapture` 与 `verify:module-size` 覆盖拆分结果。
...
Closeout：V2.73-V2.86 的 14 个单版本脚本已合并为 `scripts/verify-version-validation-docs.mjs`，`package.json` 继续保留 `pnpm verify:v2.xx-docs` 入口但统一调用参数化 verifier。旧脚本删除，checklist 历史文档保留。
...
Closeout：新增 `docs/benchmark-trends.md` 与 `verify:benchmark-trends`。已记录 `pnpm benchmark:10k`、`pnpm benchmark:task-readiness`、`pnpm benchmark:routing`、`pnpm benchmark:knowledge-search` 和 `pnpm benchmark:macos-list-model` 的 2026-06-16 本机基线；`verify:benchmark-trends` 拒绝回退到 pending benchmark 占位。
