...
| Admin skill root | Official only | Official docs list `/etc/codex/skills`; not locally validated because writing `/etc` is unnecessary for this evidence pass. Treat as read-only until product policy decides whether to scan it. |
| System/plugin skills | Partially verified | Official docs say system skills are bundled with Codex and plugins can include skills. Local `codex plugin list` and prompt input show installed plugin/bundled skills. Product mapping for plugin-distributed skills is still a separate decision. |
...
- `$CODEX_HOME/skills/codex-home-skill/SKILL.md` also appeared in prompt input locally, but this root is not listed in the official Codex skills docs as the user authoring root. Treat it as locally observed compatibility/system behavior, not an adapter root decision.
...
| --- | --- |
| `AgentId` | `codex` |
| `Scope::AgentGlobal` | `$HOME/.agents/skills/<skill-name>/SKILL.md`. Optional `/etc/codex/skills` remains out of scope and policy-gated. |
...
Do not scan or toggle plugin-distributed skills in the first adapter unless a separate product decision says plugin skills are in scope. They are distribution artifacts, not the same user/project authoring roots.
...
- Should the first Codex adapter scan `/etc/codex/skills`, or avoid admin roots to stay user-scoped?
- Should locally observed `$CODEX_HOME/skills` be scanned as a compatibility root even though official docs list `$HOME/.agents/skills` for user-authored skills?
- Should plugin-distributed skills appear in the catalog, and if so are they read-only, grouped by plugin, or omitted from V2?
