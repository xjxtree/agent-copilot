...
>
> 真实本机 app 验证在 2026-06-13 对 V2.69 重跑：multi-agent V2.69 review、Swift/native focused checks、native layout checks、screenshot artifact verification、no-capture fixture smoke、`pnpm check:privacy` 与 `git diff --check` 已通过；full `pnpm check:macos` 通过 build/test/service 阶段后在 fixture capture 阶段因 `locked-session: macOS session is locked; refusing to create screenshot evidence` fail-closed，因此未提交 fresh V2.69 screenshot。Computer Use 返回 `timeoutReached`，`ioreg` 报告 `CGSSessionScreenIsLocked=Yes`，direct capture 以 `locked-session` 退出。V2.69 capture helper 已拒绝 locked-session、black-capture 与 flat-capture。真实本机 Computer Use 仍需在解锁会话中补验；若被锁屏、AX/window resolution、Screen Recording permission 或工具层问题阻塞，必须记录 blocker，不能用 smoke 截图替代真实交互验证。
...
| V2.70 | Swift / Rust feature modularization | planned：拆分 `DetailView.swift`、`SkillStore.swift`、`crates/service/src/lib.rs` 等高负载文件，按 feature module 管理 models/actions/decoding/rendering；默认不改变 service method 语义。 |
...
1. V2.70：拆分 Swift 与 Rust 大文件，把 task、knowledge、cleanup、provider、session、lifecycle 等功能边界落到模块结构里，降低后续版本修改风险。
