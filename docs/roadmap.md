...
>
> 最新真实本机 app 验证在 2026-06-15 对 V2.79 收口：current `dist/SkillsCopilot.app` 以 exact bundle path/PID targeting 启动，unlocked Computer Use 验证 PID `68064`、`skills-copilot.main-window`、localized Work surfaces、`skills-copilot.validation-workbench`、`skills-copilot.validation-workbench.summary`、`skills-copilot.validation-workbench.evidence-standards` 与 canonical blocker rows，并记录 app-window evidence at `docs/ui-artifacts/v2.79-privacy-localization/completed.png`。泛化截图捕获曾因旧 checkout 同名窗口 fail closed as `stale-bundle`，随后 exact PID capture 成功。后续 user-visible UI 版本（尤其 V2.80）仍需新的 unlocked real-local Computer Use 验证；若被锁屏、AX/window resolution、Screen Recording permission 或工具层问题阻塞，必须记录 canonical blocker，不能用 smoke 截图替代真实交互验证。
...
V2.80-V2.83 remain planned. They become completed only after code, tests, docs, and required validation evidence prove the behavior. V2.80 specifically requires fresh unlocked real-local Computer Use validation because it affects user-visible evidence surfaces.
