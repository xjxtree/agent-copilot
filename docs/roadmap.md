...
> Agent Copilot M1-M4 decision-first IA / local evidence previews 已实现为只读感知层；当前 app 入口收敛为选中 agent 的 Agent Workspace 聚合页，组合 Agent Profile、Task Preflight、MCP Sources preview 与最后的当前 agent 本地报告导出，agent 选择放在侧边栏顶卡。侧边栏 Work 区域已移除；Provider Observability 迁移到 Settings，默认展示数据看板，并提供 Logs 选项卡按状态、provider、model、destination、问题调用和搜索词筛选。Safe Batch 入口收敛到当前技能列表标题处的批量操作按钮，通过多选 sheet 先预览再确认应用。Guided Cleanup、Validation Workbench、Skill Map 与全局 Review 已退出现行用户入口；选中技能的 Review 仍保留在技能详情切换器。显示层品牌和主要 macOS bundle 为 Agent Copilot；Swift/Rust 模块、sidecar、AX ids、env vars 和 legacy Keychain service 保持兼容。新的 adapter config writes、network-backed installs、script execution、signing、packaging 或 release automation 必须另起 scoped version。
...
| Agent Copilot M1-M4 + V2.88-V2.96 closeout | Current Agent Workspace aggregate entry for Agent Profile + Task Preflight + MCP Sources preview + final current-agent local report export, sidebar agent selector, skill-list Batch operation sheet for preview-first selected-skill enable/disable, Settings-owned Provider Observability with Dashboard/Logs tabs, retired sidebar Work section / Guided Cleanup / Validation Workbench / Skill Map / global Review user-facing entries, default-off local session preview, default-off MCP Sources preview, per-surface evidence closeout, brand asset refresh, compatibility-first identifier migration, local model-task history, Codex expanded root diagnostics, opencode configured local roots, Pi install/compat writes, Hermes native-root install, and OpenClaw native/workspace install | Completed; unlocked app-window evidence captured |
...
- V2.94 Pi install and compatibility-root writes is tracked in
...
| V2.38 | Hermes external roots | 已完成：仅把 `skills.external_dirs` 建模为 explicit external roots，不当作 project roots；V2.95 后 native-root install-only，external_dirs writes 与 config toggles 仍 blocked。 |
