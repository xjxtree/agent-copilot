...
>
> 真实本机 app 验证在 2026-06-13 对 V2.71 重跑：multi-agent V2.71 review、Rust guided-cleanup/protocol/dispatch checks、Swift tests、native layout checks、screenshot artifact verification、rebuilt no-capture fixture smoke、`pnpm check:privacy` 与 `git diff --check` 已通过；full `pnpm check:macos` 通过 build/test/service 阶段后，在 fixture capture 阶段以 `locked-session: macOS session is locked; refusing to create screenshot evidence` fail-closed。Computer Use 返回 `timeoutReached`。不提交 fresh V2.71 screenshot；真实本机 Computer Use 仍需在解锁会话中补验。若被锁屏、AX/window resolution、Screen Recording permission 或工具层问题阻塞，必须记录 blocker，不能用 smoke 截图替代真实交互验证。
...
| V2.72 | Validation harness hardening | planned：加固锁屏检测、黑屏截图检测、fixture/real screenshot matrix、Computer Use blocker 分类与 capture 脚本失败口径；锁屏或全黑截图不得作为 completed UI evidence。 |
...
3. V2.72：把 AX 窗口解析失败、fixture smoke 与 real local validation 的证据口径继续写入脚本和文档，扩展 V2.69/V2.71 已记录的锁屏/黑屏拒绝，避免把无效截图误判为 UI 验证通过。
