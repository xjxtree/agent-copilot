...
>
> 真实本机 app 验证在 2026-06-11 对 V2.52 mainline 重跑：显式选择当前 `dist/SkillsCopilot.app` bundle，focused Rust/Swift checks、`pnpm check:macos`、fixture screenshot inspection 与 `pnpm check:privacy` 已通过；真实 app launch 成功，直接 CG window capture 能找到 app window；但 Computer Use 仍返回 `cgWindowNotFound`（绝对 bundle path）与 `timeoutReached`（app name），作为工具层 blocker 保留。真实本机截图未提交，因为 live UI 暴露本地路径。后续用户可见、UI 或 service protocol 变更仍需重跑，且不能用 smoke 截图替代真实交互验证。
...
| V2.58 | Impact Preview | Planned/in progress: local-only deterministic `remediation.previewImpact` preview of impacted tasks, agents, skills, risk deltas, snapshot/rollback plan, writable capability/filtering/blockers, and evidence refs before enable/disable/edit/remediation actions; no apply/write/snapshot mutation/triage/script/credential/cloud/telemetry/default-provider side effects, and any provider wording stays V2.42 gated copy-only |
