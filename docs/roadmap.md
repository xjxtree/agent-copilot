...
>
> 真实本机 app 验证在 2026-06-12 对 V2.67 重跑：focused Rust/Swift/protocol checks、full service tests、service protocol fixture decode、`pnpm check:macos`、`pnpm check:privacy`、`git diff --check`、`git diff --cached --check` 与 fixture screenshot inspection 已通过；fixture macOS smoke 能启动并捕获当前 `dist/SkillsCopilot.app` 窗口。真实本机验证中当前 bundle 进程可启动，direct capture helper 找到 app 窗口，但 System Events 仍报告 0 AX windows，Computer Use 对 absolute bundle path 返回 `cgWindowNotFound`；作为 V2.67 window/tool-layer blocker 记录。真实本机截图未提交，因为 live UI 会暴露本地路径。后续用户可见、UI 或 service protocol 变更仍需重跑，且不能用 smoke 截图替代真实交互验证。
...
| V2.68 | Task Cockpit 主入口 / Analysis IA 重组 | planned：把 task-first cockpit 作为默认工作入口，Analysis 拆为 Task Cockpit、Skill Map、Cleanup Flow、Observability 等可理解区域；默认复用 existing service methods，不新增 provider 默认请求、hidden task state 或写入路径。 |
...
1. V2.68：把 Task Cockpit 提升为主入口，让用户从“我要做什么任务”开始，而不是先在多个 Analysis 面板里寻找功能。
2. V2.69：提供 privacy/screenshot mode，解决真实本机截图会暴露本地路径的问题，并同步清理中英文混排、过长路径和过密标签。
3. V2.70：拆分 Swift 与 Rust 大文件，把 task、knowledge、cleanup、provider、session、lifecycle 等功能边界落到模块结构里，降低后续版本修改风险。
4. V2.71：让 Guided Cleanup 连接到既有安全预览入口，形成可执行但不隐藏 apply 的复查路径。
5. V2.72：把锁屏、黑屏、AX 窗口解析失败、fixture smoke 与 real local validation 的证据口径写入脚本和文档，避免把无效截图误判为 UI 验证通过。
