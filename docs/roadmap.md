...
> 不代表当前 MVP/V1 进度遗漏。
>
> 当前阶段：**V2.94 Pi install and compatibility writes completed**。
> V2.1-V2.94 是当前 completed baseline；V2.87 于 2026-06-17 解锁后通过 `pnpm check:macos`，V2.88 补齐 Lineup / Agent Profile / Local Session Preview / MCP Preview 的 Computer Use app-window evidence，V2.89 刷新 Agent Copilot display brand app icon assets，V2.90 完成兼容优先的 packaged app identity / default app-data id 迁移，V2.91 新增本地模型-任务匹配历史证据域，V2.92 完成 Codex expanded roots 只读诊断和 native-root write allowlist，V2.93 完成 opencode configured local `skills.paths` scanning 并保持 `skills.urls` metadata-only/no-fetch，V2.94 完成 Pi `.agents/skills` compatibility scanning/toggles 与 native-root installs。
> Agent Copilot M1-M4 decision-first IA / local evidence previews 已实现为只读感知层，显示层品牌和主要 macOS bundle 为 Agent Copilot；Swift/Rust 模块、sidecar、AX ids、env vars 和 legacy Keychain service 保持兼容。Remaining near-term follow-ups are scheduled as V2.95-V2.96 slices below and in [`development-tasks.md`](./development-tasks.md).
...
| Agent Copilot M1-M4 + V2.88-V2.94 closeout | Decision-first Lineup, Agent Profile, sorted decision queue, default-off local session preview, default-off MCP server preview, per-surface evidence closeout, brand asset refresh, compatibility-first identifier migration, local model-task history, Codex expanded root diagnostics, opencode configured local roots, and Pi install/compat writes | Completed; unlocked app-window evidence captured |
...
- V2.90 Agent Copilot identifier migration is tracked in
...
- V2.93 opencode custom local roots is tracked in
  [`v2.93-verification-checklist.md`](./v2.93-verification-checklist.md).
...
**V2 剩余开发判定**
- 当前 adapter baseline：Pi guarded native/global/project/package toggles、`.agents/skills` compatibility toggles、native-root installs 已完成，Pi package install/remove 与 `.agents` direct install 继续 blocked；opencode guarded `permission.skill` writes、native install 和 configured local `skills.paths` read-only scanning 已完成，`skills.urls` 保持 metadata-only/no-fetch；Hermes/OpenClaw 保持 read-only，writable/install 分别排入 V2.95/V2.96 前继续 blocked。
...
| V2.29 | Finding triage persistence（completed） | 支持 reviewed / ignored / needs-follow-up，本地 catalog 持久化，不写 agent config；finding fingerprint 变化后重新打开。 |
| V2.30 | AI Skill Analysis 工作流增强（completed） | 用户触发（selected/batch）只读预览；默认 disabled-by-default，先本地 prepare/preview，不触发 provider 调用；支持总结、风险解释、清理建议草稿（copy-only）；不写文件、不触发 agent-config 写入、不建 snapshot、不执行脚本、不保存凭据。 |
...
### 4.29-V2.29 Finding triage persistence（completed）

- 目标：支持 reviewed / ignored / needs-follow-up 的本地 finding triage 状态，让用户区分已知问题与新增 actionable findings。
- 边界：triage 状态仅写入 app-local catalog/app data，不写 agent config；持久状态仅为 `Open` / `Reviewed` / `Ignored` / `Needs follow-up`。finding fingerprint 或受影响实例集合（signature）变化后，已 triage 的 finding 自动回到 `Open`。
- 状态不走任何 agent-config 持久化路径，不创建 skill-toggle snapshot 或 skill-content snapshot。
- triage 操作不得触发脚本执行、AI 写回、provider 调用或凭据保存。

### 4.30-V2.30 AI skill analysis workflow（completed）

- 目标：把可解释性向 cleanup 决策延伸，但不扩大写入/执行边界。
- 范围：
  - 用户显式触发 `selected` 或 `batch` 分析预览；不提供后台自动触发。
  - 默认 `disabled-by-default`、read-only，优先本地 prepare/preview；除非未来显式 opt-in 开通，否则不发起 provider 网络调用。
  - 返回内容包含 summary、risk explanation、cleanup/suggestion draft，草稿仅供复制/展示，不可直接 apply。
...
该阶段已完成 read-only 诊断字段：为每个 agent 显式对齐 `roots discovered / skipped / blocked`、`config detected`、`read-only / writable` 原因、以及 `catalog.scanAll` 最近 activity 时间与状态。V2.40 不新增写入、install、script execution、AI 自动写回、credentials 或 telemetry。
...
| V2.38 | Hermes external roots | 已完成：仅把 `skills.external_dirs` 建模为 explicit external roots，不当作 project roots；保持 read-only，writable/install blocked。 |
...
| V2.49 | Routing Accuracy Dashboard | 已完成：展示 hit/miss/wrong-pick/ambiguity/unknown、accuracy rate、benchmark gap、regression 与 recent evidence 指标，按 agent 和时间 bucket 汇总。 |
| V2.50 | Cross-agent Task Readiness | 已完成：同一任务横向比较 Claude/Codex/opencode/Pi/Hermes/OpenClaw 的 skill 可见性、质量、风险与 routing confidence；`task.compareAgentReadiness` 默认 read-only、deterministic、no-write、no-provider。 |
| V2.51 | Stale / Drift Detection | 已完成：以 `analysis.detectStaleDrift` 作为只读入口，基于 fingerprint、mtime、finding/conflict/analysis/source drift 和 readiness impact 标记过期或行为漂移 skills；不默认 provider，不写入。 |
| V2.52 | Local Knowledge Index | 已完成：`knowledge.search` 与 native Analysis UI 提供 local-only 只读检索，按任务、用途、工具、关键词、规则、来源、agent、风险、quality/readiness/stale-drift context 和能力域对现有 catalog evidence 做搜索；默认不联网、不发 provider 请求、不写 index artifact。 |
| V2.53 | Similar Skill Grouping | 已完成：`knowledge.groupSimilarSkills` 与 native Analysis UI 基于 existing catalog evidence、V2.52 tags、source/name/tool/rule/capability/risk overlaps、quality/readiness/stale-drift context 生成 duplicate/similar/confusable groups，区分 coverage redundancy / routing ambiguity；默认不联网、不发 provider 请求、不写 group artifact。 |
| V2.54 | Capability Taxonomy | 已完成：`knowledge.buildCapabilityTaxonomy` 与 native Analysis UI 基于 existing catalog evidence、V2.52 tags、V2.53 similar groups、quality/stale-drift context、agent/workspace/source/rule/tool/risk signals 生成能力域、覆盖、缺口、重复/冗余与 routing ambiguity；默认不联网、不发 provider 请求、不写 taxonomy artifact。 |
| V2.55 | Workspace Readiness Check | 已完成：`workspace.checkReadiness` 与 native Analysis UI 基于本地 catalog/taxonomy/readiness/routing/cross-agent/stale-drift/adapter evidence 生成 workspace checklist、agent rows、capability rows、gap/blocker notes、evidence refs、prompt request metadata 与 safety flags；默认不发 provider、不写 artifact、不新增写入路径。 |
| V2.56 | AI Remediation Planner | 已完成：`remediation.plan` 与 native Analysis UI 将 findings、gap、routing ambiguity、drift、readiness、taxonomy、workspace evidence 转成优先级明确的只读处理计划；native UI 已集成。 |
| V2.57 | Fix Preview Drafts | 已完成：`remediation.previewDrafts` 与 native Analysis UI 基于本地 findings、skill metadata/frontmatter/body snippets、remediation plan、conflicts/analysis/rules/provenance 生成 frontmatter、description、permissions、dependency、policy 的 copy/edit-ready draft suggestions；默认不发 provider、不写 artifact、不提供直接 apply/write 路径。 |
| V2.58 | Impact Preview | 已完成：`remediation.previewImpact` 与 native Analysis UI 基于本地 remediation/readiness/routing/drift/taxonomy/workspace/writable evidence 预览 impacted tasks、agents、skills、risk deltas、snapshot/rollback plan、writable capability/filtering/blockers 与 evidence refs；不 apply、不写 skill/config、不创建/回滚 snapshot、不发 provider。 |
...
| V2.62 | Agent Session Skill Review | 已完成：`session.reviewAgentSkillUse` / `session.listSkillReviews` / `session.deleteSkillReview` 从 V2.48 imported trace、用户粘贴/导入的 agent transcript，或未来显式选择的本地 agent 会话创建/读取/删除只读 review session；审查真实 agent 是否正确发现/选择/使用 skill、是否漏用更合适 skill、是否被相似/重复 skill 干扰，并生成 app-local `agent-session-reviews.json` redacted metadata、safe next steps 与 evidence refs。本 app 的 AI prompt run history 只作为辅助证据，不作为主审查对象；不保存 raw transcript/raw prompt/raw response/secrets/unredacted paths，不写 skill/config，不改 triage/snapshot，不执行脚本，不发 provider/cloud/telemetry。 |
| V2.63 | Local Skill Map | 已完成：`knowledge.buildLocalSkillMap` 基于 existing catalog evidence、knowledge tags、similar groups、capability taxonomy、conflicts、cross-agent analysis、task coverage、readiness/routing/session-review context、stale/drift、source provenance 与 risk evidence 本地生成 skill map；用户触发、deterministic/read-only、no new source of truth、默认不持久化 artifact、不发 provider、不写 skill/config、不改 snapshot/triage、不执行脚本。 |
| V2.64 | AI Provider Observability | 已完成：`llm.providerObservability` 基于 V2.61 prompt run metadata 与 existing minimal provider call metadata 派生 summary、call/history rows、provider/model/destination grouping rows、status rows、budget usage hints、retention recommendations、evidence refs、prompt metadata 与 safety flags；用户触发、deterministic/read-only、app-local evidence only；不存 raw prompt/raw response JSON/API keys/credentials/raw traces/unredacted paths，不写 skill/config，不改 triage/snapshot，不执行脚本，不默认发 provider/network request，不云同步/telemetry。 |
| V2.65 | Task-first Cockpit | completed：以用户任务为入口，把 task readiness、routing confidence、benchmark/regression、trace/session review、remediation next steps 与 evidence refs 聚合成一个任务工作台；默认 local/read-only，不创建新写入路径或 hidden task state。 |
| V2.66 | Skill Lifecycle Timeline | completed：`skill.lifecycleTimeline` 以单个 skill / agent / workspace 为维度展示 discovered、changed、stale/drift、triage、remediation decision、prompt run/provider observability 和 session review evidence 的时间线；从 existing local evidence 派生，不默认持久化 new raw lifecycle artifacts，不写 skill/config/triage/snapshot，不发 provider request。 |
| V2.67 | Guided Cleanup Flow | completed：`cleanup.planGuidedFlow` 把 findings、cleanup queue、similar groups、stale/drift、readiness/routing/task cockpit、lifecycle timeline、remediation plan/drafts/impact/batch review、adapter diagnostics、source provenance 串成可跟进流程；`cleanup.recordGuidedStep` 只写 app-local redacted metadata；任何实际 enable/disable/edit/remediation 仍只能走既有 preview-first、explicit-confirm、安全写入路径。 |
| V2.68 | Task Cockpit 主入口 / Analysis IA 重组 | completed：Task Cockpit 是默认工作入口，sidebar Work surfaces 前置到诊断卡片之前，Detail section switcher 改为 bounded menu，Analysis 拆为 Task Cockpit、Skill Map/Lifecycle、Guided Cleanup、Provider Observability、Review；默认复用 existing service methods，不新增 provider 默认请求、hidden task state 或写入路径。 |
| V2.69 | Privacy / Screenshot Mode + 本地化收束 | completed：默认截图隐私模式、路径脱敏/长路径折叠与 reveal-on-demand、Task Cockpit / Guided Cleanup / Provider Observability 文案本地化、`app_language` 传递、截图 artifact verifier、锁屏/黑屏 capture 拒绝已完成；不改变 scanner/catalog 事实层。 |
| V2.70 | Swift / Rust feature modularization | completed：拆出 `TaskCockpitPanel.swift`、`DetailPresentationPrimitives.swift` 与 Rust `cleanup_queue.rs`，并更新 native layout verifier 聚合拆分后的 UI 文件；默认不改变 service method / payload / persistence / provider / write 语义。 |
| V2.71 | Guided Cleanup safe-action deep links | completed：guided cleanup step/action 暴露 safe deep-link metadata，并在 Swift UI 中跳转到现有 cleanup/detail/remediation/lifecycle/cockpit/safe batch preview/guided metadata 入口；不隐藏 apply，不绕过 preview-first / explicit-confirm。 |
| V2.72 | Validation harness hardening | completed：统一 validation blocker taxonomy、增加 classifier CLI、smoke 锁屏 preflight、截图 verifier canonical blocker 输出、fixture/real screenshot matrix 与 closeout checklist；锁屏或全黑截图不得作为 completed UI evidence。 |
| V2.73 | Task / remediation performance and timeout recovery | completed：真实 catalog 下 task/remediation 聚合有 bounded metadata、timeout/fallback diagnostics、cancel/retry UI 和 unlocked real-local evidence；不新增 provider/write/execute 语义。 |
| V2.74 | Real-local launch and window targeting stability | completed：current bundle path/PID/window identity targeting、duplicate same-bundle fail-closed、AX/Computer Use evidence 与 app-window screenshot evidence 已收口；不新增 distribution/signing 语义。 |
| V2.75 | Task input and input-method resilience | completed：AX-settable multiline Task Cockpit input、exact nonblank text preservation、whitespace-only submit blocking、explicit Build submit 与 real-local evidence 已收口；不新增 raw prompt/provider/write/execute/cloud/telemetry 语义。 |
| V2.76 | Progressive Cockpit feedback | completed：readiness/routing/cross-agent/remediation/provider/session staged feedback、partial rows、elapsed time、timeout/fallback/blocked/skipped states、PID `39728` real-local evidence、`skills-copilot.task-cockpit.stage-progress` 和截图证据已收口；不新增 provider/write/execute/credential/cloud/telemetry 语义。 |

### V2.70-V2.72 产品收束方向（完成）

后续版本以降低用户和维护者的操作成本为中心，不再优先叠加新的分析端点：

1. V2.70：已拆分 Swift Task Cockpit/detail presentation primitives 与 Rust cleanup queue，降低后续版本修改风险。
...
| V2.82 | Test isolation and core model test floor | Completed: provider env mutation tests use serialized RAII cleanup; core model tests lock `AgentId` / `Scope` wire strings, `PermissionRequest` safe defaults, and identity/state fields without adding serde dependencies | Test hygiene only; no provider credential persistence changes, service protocol method/payload changes, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads beyond existing explicitly confirmed provider tests, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP |
| V2.83 | Continued module splitting | Completed: split Rust protocol DTOs/constants, Swift Detail overview helpers, and Swift fake service test helpers into dedicated files; updated split-file protocol/native UI verifiers | Refactor-only; no service protocol method or payload changes, no protocol version bump, no new UI surface, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP |
| V2.84 | Swift Detail section splitting | Completed: `DetailView.swift` is now a router/composer; split section files including `DetailGuidedCleanupFlowPanel.swift`, `DetailProviderObservabilityPanel.swift`, and `DetailReviewCoreSection.swift` preserve Task Cockpit / Skill Map / Guided Cleanup / Provider Observability / Review semantics; `verify:module-size` guards file size | Refactor-only; no new UI behavior, service protocol method or payload changes, provider default calls, write/apply paths, script execution, credentials, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP |
| V2.85 | Rust RPC domain module splitting | Completed: `ServiceHost` RPC implementation moved into `service_host.rs`, `service_cleanup.rs`, `service_knowledge.rs`, `service_llm.rs`, `service_remediation.rs`, and `service_task.rs`; protocol drift verifier scans split service files | Refactor-only; no service protocol method or payload changes, no protocol version bump, no provider/write/script/cloud/telemetry expansion |
| V2.86 | Rust helper/test split and module-size gate | Completed: helper files including `service_support_helpers.rs` and test chunks under `crates/service/src/tests/` are checked by `pnpm verify:module-size`, now part of `pnpm verify:gate-parity` | Refactor-only validation hardening; no service protocol/UI/provider/write/script/cloud/telemetry expansion |

V2.81, V2.82, V2.83, V2.84, V2.85, and V2.86 are completed with code, focused tests, docs, shared gates, and documented no-new-UI validation decisions where applicable.
...
V2.76 closeout evidence is tracked in [`v2.76-verification-checklist.md`](./v2.76-verification-checklist.md) and guarded by `pnpm verify:v2.76-docs`. It records completed Progressive Cockpit feedback: readiness/routing/cross-agent/remediation/provider/session staged feedback, partial rows, elapsed time, timeout/fallback/blocked/skipped states, PID `39728` real-local Computer Use evidence, `skills-copilot.task-cockpit.stage-progress`, screenshot evidence, and no provider/write/execute/credential/cloud/telemetry scope expansion.
...
V2.80 closeout evidence is tracked in [`v2.80-verification-checklist.md`](./v2.80-verification-checklist.md) and guarded by `pnpm verify:v2.80-docs`. It records completed Detail navigation and visual density polish evidence: implementation code, focused tests, docs, shared gates, PID `82571` real-local Computer Use, `docs/ui-artifacts/v2.80-detail-density/completed.png`, and safety-boundary confirmation. V2.80 must not add service methods, provider defaults, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, or public distribution automation.

V2.81 closeout evidence is tracked in [`v2.81-verification-checklist.md`](./v2.81-verification-checklist.md) and guarded by `pnpm verify:v2.81-docs`. It records completed Swift stdio sidecar cancellation cleanup, focused Swift cancellation and force-kill tests, shared gate wiring, no fresh Computer Use screenshot required because no user-visible UI changed, and this boundary: no daemon/socket redesign by default, service protocol method/payload changes, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution, signing/notarization/DMG/ZIP.

V2.82 closeout evidence is tracked in [`v2.82-verification-checklist.md`](./v2.82-verification-checklist.md) and guarded by `pnpm verify:v2.82-docs`. It records completed provider env mutation RAII isolation, core model wire/default/identity test floor, focused tests, shared gates, no-capture fixture smoke, no fresh Computer Use screenshot required because no user-visible UI or service-protocol behavior changed, and the current canonical `locked-session` blocker for `pnpm check:macos` / `./script/build_and_run.sh --verify` UI evidence capture. Boundary: no provider credential persistence changes, service protocol method/payload changes, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads beyond existing explicitly confirmed provider tests, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP.

V2.83 closeout evidence is tracked in [`v2.83-verification-checklist.md`](./v2.83-verification-checklist.md) and guarded by `pnpm verify:v2.83-docs`. It records completed continued module splitting: split Rust protocol DTOs/constants, split Swift Detail overview helpers, split Swift fake service test helpers, focused Rust protocol checks, Swift tests, shared gates, no-capture fixture smoke, no fresh Computer Use screenshot required because no user-visible UI or service-protocol behavior changed, and the current canonical `locked-session` blocker for `pnpm check:macos` UI evidence capture. Boundary: no service protocol method or payload changes, protocol version bump, new UI surface, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP.
...
1. Multi-agent V2.72 analysis completed across capture scripts, screenshot artifact verification, documentation state, and validation command matrix.
2. Added `scripts/validation-blockers.mjs` canonical blocker taxonomy and `pnpm classify:validation-blocker` for Computer Use / AX / capture text classification.
3. Added `pnpm verify:validation-blockers` and wired it into `pnpm check:macos`.
4. `scripts/smoke-macos-app.mjs` now preflights lock-screen state before accepting window capture evidence and emits canonical blocker labels for stale bundle/window/capture failures.
5. `script/capture_app_window.sh` and `scripts/verify-screenshot-artifacts.mjs` use canonical blocker labels for locked-session, black-capture, flat-capture, transparent-capture, invalid-capture, screen-recording-permission, and window-not-found cases.
6. Focused checks passed: `pnpm verify:validation-blockers`, `pnpm classify:validation-blocker -- "Computer Use server error -10005: timeoutReached"`, synthetic black PNG rejection, `pnpm verify:screenshot-artifacts`, `pnpm smoke:macos-app -- --fixture-data`, `pnpm check:privacy`, and `git diff --check`.
7. Current locked-session evidence: `pnpm smoke:macos-app -- --fixture-data --capture-window` and full `pnpm check:macos` fail closed with `locked-session`; Computer Use returns `timeoutReached`; no fresh real-local screenshot was committed.

### V2.71 Verification Checklist（完成）

1. Multi-agent V2.71 analysis completed across service/protocol safety, Swift UI routing, and validation/docs risk.
2. Service emitted `safe_action_deep_link` for flow steps and `deep_link` for safe next actions; protocol fixtures decode these fields with no-apply safety flags.
3. Native UI renders guided cleanup safe-link buttons and routes only to existing read-only / preview-first surfaces. Analysis now mounts remediation plan, fix preview drafts, impact preview, batch review, and remediation history panels as link destinations.
4. Safe batch links open the existing safe batch preview context only; no auto-preview, apply, toggle, script execution, provider confirmation, snapshot, triage mutation, or config write is triggered.
5. Focused checks passed: `cargo fmt --all -- --check`, `cargo test -p skills-copilot-service --lib guided_cleanup`, `cargo test -p skills-copilot-service --lib service_protocol_fixtures_decode`, `cargo test -p skills-copilot-service --lib supported_methods_have_dispatch_coverage`, `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:screenshot-artifacts`, `pnpm smoke:macos-app -- --fixture-data`, `pnpm check:privacy`, and `git diff --check`.
6. Full `pnpm check:macos` passed build/test/service stages and failed closed at fixture capture with `locked-session: macOS session is locked; refusing to create screenshot evidence`.
7. Computer Use returned `timeoutReached`; no fresh V2.71 screenshot was committed. Unlocked visual validation remains required.

### V2.70 Verification Checklist（完成）
...
Closeout status: completed; subsequent V2.71 work added Guided Cleanup safe-action deep links without hiding apply or bypassing preview/confirm flows.

### V2.69 Verification Checklist（完成）

1. Multi-agent V2.69 analysis completed across scope calibration, UI/localization, and privacy/screenshot validation.
2. Localized resource syntax passed: `plutil -lint apps/macos/Sources/SkillsCopilot/Resources/en.lproj/Localizable.strings apps/macos/Sources/SkillsCopilot/Resources/zh-Hans.lproj/Localizable.strings`.
...
6. No fresh V2.69 screenshot was committed; the V2.69 rule is that locked/black/flat capture output is a blocker, not visual evidence.
7. `pnpm check:privacy`, `git diff --check`, and `git diff --cached --check` passed.
8. Real-local Computer Use remains pending until an unlocked interactive macOS session is available; locked-session, black-capture, flat-capture, window-not-found, Screen Recording permission, or tool-layer errors must be recorded as blockers.

Closeout status: completed with explicit locked-session capture blocker; V2.70 should focus on Swift/Rust feature modularization without changing protocol semantics.

### V2.68 Verification Checklist（完成）

1. Multi-agent V2.68 analysis completed across UI/IA, service/protocol, and verification/docs scopes.
2. Focused Rust checks passed: `task_cockpit`, `service_protocol_fixtures_decode`, and `guided_cleanup`.
3. Swift tests passed: `swift test --package-path apps/macos`.
4. Native layout check passed: `pnpm verify:macos-ui-layout`, including bounded menu navigation, hidden picker label, Work surfaces, and Task Cockpit-before-empty fallback.
5. Full gate passed: `pnpm check:macos`, including fixture app-window capture.
6. Fixture screenshot inspection passed: Task Cockpit is selected by default and Work surfaces appear before Adapter/Health diagnostics.
7. Real local launch succeeded and CG window metadata found `SkillsCopilot`, but the macOS session was locked (`CGSSessionScreenIsLocked=Yes`), Computer Use timed out, and the final direct capture was black. This is the V2.68 locked-session/window-capture blocker.
8. `pnpm check:privacy` and `git diff --check` passed.

Closeout status: completed; V2.69 should address screenshot/privacy and locked/black capture handling.
...
1. Focused Rust/Swift checks：`cargo test --workspace`、`cargo clippy --workspace --all-targets --all-features`、`swift test --package-path apps/macos`。
2. `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run` / `pnpm dev:macos`) with explicit `SkillsCopilot` window `Computer Use`/AX observation; if window resolution still fails, record blocker.
4. `pnpm check:privacy`。
5. App-window-only screenshot and manual redaction inspection for local path/credential placeholders.
6. 复核 V2.43 口径：质量评分仅允许 selected/batch 触发；仅读结果；默认不发起后台任务；若使用 provider，仅在完成 preview/确认流程后才执行，且 preview 必须显示 redaction、included/excluded、token/cost、destination。
7. 复核关闭边界：不改变 agent-config、snapshot、triage、toggle/save、script execute；不触发 AI 自动 write-back。
8. 复核数据源边界：评分解释输入仅为本地 evidence（metadata/findings/conflicts/analysis/adapter diagnostics）；prompt/raw prompt 或 response 不默认持久化。

Closeout status: completed. V2.43 integrates protocol + service + UI closeout and evidence for the same boundaries: focused Rust/Swift checks, `pnpm check:macos`, real local Computer Use validation, fixture screenshot inspection, and `pnpm check:privacy`.

### V2.44 Verification Checklist（完成）

1. Focused Rust/Swift checks：`cargo test --workspace`、`cargo clippy --workspace --all-targets --all-features`、`swift test --package-path apps/macos`。
2. `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run` / `pnpm dev:macos`) and explicit `SkillsCopilot` window Computer Use / AX attempt；若窗口不可见，记录 blocker。
4. `pnpm check:privacy`。
5. App-window-only 截图并手工核查路径/凭据占位符脱敏。
6. 复核 V2.44 口径：当前主线已在 service/UI 中暴露 `task.checkReadiness`；用户输入真实任务后，本地 deterministic readiness 使用 metadata/findings/conflicts/analysis/adapter diagnostics/V2.43 quality 生成 read-only 结果；无 background readiness 轮询。
7. 复核关闭边界：无 provider 触发脚本执行、无 triage mutation、无 config/snapshot mutation、无 new raw prompt/response 持久化。
...
1. Focused Rust/Swift checks：`cargo test --workspace`、`cargo clippy --workspace --all-targets --all-features`、`swift test --package-path apps/macos`。
2. `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run` / `pnpm dev:macos`) 并对当前 `dist/SkillsCopilot.app` bundle 进行 V2.46 目标流程的 `Computer Use`/AX 验证；若窗口不可见，记录 blocker。
4. `pnpm check:privacy`。
5. App-window-only 截图并手工复核路径/凭据占位符脱敏。
6. 复核 V2.46 口径：
   - 用户可维护 app-local task benchmark set（task、expected skill refs/names、acceptable agent/scope、成功标准）；
   - 已实现 `task.listBenchmarks` / `task.saveBenchmark` / `task.deleteBenchmark` / `task.evaluateBenchmarks`；
   - benchmark 评估以 `metadata` / `findings` / `conflicts` / `analysis` / `adapter diagnostics` / `quality_score` / `task.checkReadiness` / `task.rankSkillRoutes` 为主；
   - 可选 provider 说明依旧走 V2.42 `llm.previewPrompt` + `llm.confirmPromptAndSend`，保持 preview/redaction/确认和 copy-only。
7. 复核关闭边界：本地 benchmark 不得发起 provider 请求、不得改 triage，不得读 credentials，不得变更 agent config，不得创建 snapshot，不得执行脚本。
...
5. App-window-only 截图并手工复核路径/凭据占位符脱敏。
6. 复核 V2.47 口径：
   - 比较对象为 V2.46 benchmark 结果和 app-local baseline（`task.saveRoutingBaseline` / `task.detectRoutingRegression`）；输出需给出 score/confidence delta、状态变化、top-route 变化、missing benchmark 与回归项。
   - 回归指标默认基于本地证据（`TaskReadiness` + `TaskBenchmark` + `quality score`/`routing confidence`/`analysis` 证据链）和 app-local 结果，不调用 provider scoring。
   - optional provider 辅助说明仍走 V2.42 的 `llm.previewPrompt` + `llm.confirmPromptAndSend`，保持 copy-only，不改变路由判断。
...
2. `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run` / `pnpm dev:macos`) 并对当前 `dist/SkillsCopilot.app` bundle 进行 routing accuracy dashboard 的 `Computer Use`/AX 验证；若窗口不可见，记录 blocker。
4. `pnpm check:privacy`。
5. Fixture screenshot 手工核查路径/凭据占位符脱敏；真实本机截图不得提交。
6. 复核 V2.49 口径：
   - `routing.accuracyDashboard` 聚合 `trace.importLocal` outcomes、`task.evaluateBenchmarks` 结果与 `task.detectRoutingRegression` evidence；
   - 输出 `summary`、`agent_rows`、`history_rows`、`gap_issue_rows`、`recent_evidence_rows`、`blocker_notes`、`prompt_request`、`safety_flags`；
   - dashboard 只读、user-triggered、local evidence-first，不写新 dashboard artifact；
   - 本地路径不落 raw trace，不发 provider 请求，不改 triage/config/snapshot/skill 文件，不读 credentials，不执行脚本，不做 cloud sync/telemetry。
7. 复核 provider 辅助路径：仍需 V2.42 `llm.previewPrompt` + `llm.confirmPromptAndSend`，输出 copy/display-only，不改变 deterministic dashboard 结果。

**Closeout status**: completed with explicit Computer Use blocker. V2.49 integrates `routing.accuracyDashboard`, deterministic local accuracy aggregation, native Analysis dashboard UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, and `pnpm check:privacy`. Real local app launch against the current bundle succeeded and direct window capture found the app window, but Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.50 Verification Checklist（完成）

1. Focused Rust/Swift checks：`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run` / `pnpm dev:macos`) 并对当前 `dist/SkillsCopilot.app` bundle 进行 V2.50 目标流程的 `Computer Use`/AX 验证；若窗口解析失败，明确记录 blocker。
4. `pnpm check:privacy`。
5. fixture screenshot 与 App-window-only 截图手工核查路径/凭据占位符脱敏。
6. 复核 V2.50 口径：
   - 已实现 `task.compareAgentReadiness`：只读横向任务 readiness 对比（Claude/Codex/opencode/Pi/Hermes/OpenClaw）。
   - 输入依赖本地 evidence：`task.checkReadiness`、`task.rankSkillRoutes`、`task.evaluateBenchmarks`、`task.detectRoutingRegression`、`trace.importLocal`、`routing.accuracyDashboard`。
   - 输出包含 summary、agent rows、recommended agent、gap/issue rows、evidence refs、prompt request 与安全边界标志，不写新 artifact。
   - 默认不发 provider 请求；可选 provider 说明继续走 V2.42 `llm.previewPrompt` + `llm.confirmPromptAndSend`，copy-only。

**Closeout status**: completed with explicit Computer Use blocker. V2.50 integrates `task.compareAgentReadiness`, deterministic local cross-agent readiness comparison, native Analysis comparison UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, `pnpm check:privacy`, and real local launch validation. Real local app launch against the current bundle succeeded and direct window capture found the app window, but Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed because the live UI exposes local paths.
...
**Closeout status**: completed with explicit Computer Use blocker. V2.51 integrates `analysis.detectStaleDrift`, deterministic local stale/drift detection, native Analysis stale/drift UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, `pnpm check:privacy`, and real local launch validation. Real local app launch against the current bundle succeeded and direct window capture found the app window, but Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed because the live UI exposes local paths.
...
**Closeout status**: completed with explicit Computer Use blocker. V2.52 integrates `knowledge.search`, deterministic local knowledge search, native Analysis local knowledge UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, `pnpm check:privacy`, and real local launch validation. Real local app launch against the current bundle succeeded and direct window capture found the app window, but Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed because the live UI exposes local paths.
...
**Closeout status**: completed with explicit Computer Use blocker. V2.53 integrates `knowledge.groupSimilarSkills`, deterministic similar skill grouping, native Analysis similar grouping UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, `pnpm check:privacy`, and real local launch validation. Real local app launch against the current bundle succeeded and direct window capture found the app window, but Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed because the live UI exposes local paths.

### V2.54 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) after removing stale same-bundle processes; attempt current `dist/SkillsCopilot.app` bundle with direct CG capture, AX, and Computer Use.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.54 口径：`knowledge.buildCapabilityTaxonomy` 仅做 local-only、read-only、deterministic、user-triggered taxonomy；默认不发 provider 请求、不联网、不写 skill/agent config/taxonomy artifact/snapshot/triage，不执行脚本，不读 credentials，不持久化 raw prompt/response/trace。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.54 integrates `knowledge.buildCapabilityTaxonomy`, deterministic local capability taxonomy, native Analysis capability taxonomy UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded after stale same-bundle processes were removed; CG window lookup found the app window id, but direct window captures rendered black, AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.55 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service workspace_check_readiness --lib`、`cargo test -p skills-copilot-service workspace_readiness_prompt --lib`、`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; if the known window tool blocker remains, record it explicitly.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.55 口径：`workspace.checkReadiness` 仅做 local-only、read-only、deterministic、user-triggered workspace readiness；默认不发 provider 请求、不联网、不写 skill/agent config/readiness artifact/snapshot/triage，不执行脚本，不读 credentials，不持久化 raw prompt/response/trace。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.55 integrates `workspace.checkReadiness`, deterministic local workspace readiness, native Analysis workspace readiness UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded after stale same-bundle processes from older worktrees were removed; after relaunch only the current checkout process was running, but AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.56 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service remediation_plan --lib`、`cargo test -p skills-copilot-service service_protocol_fixtures_decode --lib`、`cargo test -p skills-copilot-service supported_methods_have_dispatch_coverage --lib`、`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; if the known window tool blocker remains, record it explicitly.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.56 口径：`remediation.plan` 仅做 local-only、read-only、deterministic、user-triggered remediation planning；默认不发 provider 请求、不联网、不写 skill/agent config/remediation artifact/snapshot/triage，不执行脚本，不读 credentials，不持久化 raw prompt/response/trace。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.56 integrates `remediation.plan`, deterministic local remediation planning, native Analysis remediation planner UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded and only the current checkout process was running, but AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.57 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service remediation_preview_drafts --lib`、`cargo test -p skills-copilot-service service_protocol_fixtures_decode --lib`、`cargo test -p skills-copilot-service supported_methods_have_dispatch_coverage --lib`、`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; if the known window tool blocker remains, record it explicitly.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.57 口径：`remediation.previewDrafts` 仅做 local-only、read-only、deterministic、user-triggered copy/edit draft suggestions；默认不发 provider 请求、不联网、不写 skill/agent config/draft artifact/snapshot/triage，不提供直接 apply/write，不执行脚本，不读 credentials，不持久化 raw prompt/response/trace。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.57 integrates `remediation.previewDrafts`, deterministic local fix preview drafts, native Analysis fix preview drafts UI, protocol fixtures, focused Rust/Swift tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded and only the current checkout process was running, but AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.58 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service remediation_preview_impact --lib`、`cargo test -p skills-copilot-service service_protocol_fixtures_decode --lib`、`cargo test -p skills-copilot-service supported_methods_have_dispatch_coverage --lib`、`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; if the known window tool blocker remains, record it explicitly.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.58 口径：`remediation.previewImpact` 仅做 local-only、read-only、deterministic、user-triggered impact preview；默认不发 provider 请求、不联网、不写 skill/agent config/impact artifact/snapshot/triage，不创建或回滚 snapshot，不提供直接 apply/write，不执行脚本，不读 credentials，不持久化 raw prompt/response/trace。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.58 integrates `remediation.previewImpact`, deterministic local impact preview, native Analysis impact preview UI, protocol fixtures, focused Rust/Swift tests, full service tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded and only the current checkout process was running, but AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.59 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service remediation_batch_review --lib`、`cargo test -p skills-copilot-service service_protocol_fixtures_decode --lib`、`cargo test -p skills-copilot-service supported_methods_have_dispatch_coverage --lib`、`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; if the known window tool blocker remains, record it explicitly.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.59 口径：`remediation.batchReview` 仅做 local-only、read-only、deterministic、user-triggered batch review workflow；默认不发 provider 请求、不联网、不写 skill/agent config/batch artifact/snapshot/triage，不创建或回滚 snapshot，不提供直接 apply/write，不执行脚本，不读 credentials，不持久化 raw prompt/response/trace。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.59 integrates `remediation.batchReview`, deterministic local batch review grouping, native Analysis batch review UI, protocol fixtures, focused Rust/Swift tests, full service tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded and only the current checkout process was running, but AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name. No real-local screenshot was committed.

### V2.60 Verification Checklist（完成）

1. Focused Rust/Swift checks: `cargo test -p skills-copilot-service remediation_history --lib`、`cargo test -p skills-copilot-service service_protocol_fixtures_decode --lib`、`cargo test -p skills-copilot-service supported_methods_have_dispatch_coverage --lib`、`cargo test -p skills-copilot-service -- --nocapture`、`swift test --package-path apps/macos`。
2. Full gate: `pnpm check:macos`。
3. Real local launch (`./script/build_and_run.sh run`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; known window tool blocker remains explicit.
4. Fixture screenshot inspection and manual check that committed screenshot evidence is fixture-only and does not expose real local paths.
5. `pnpm check:privacy` before commit.
6. 复核 V2.60 口径：`remediation.listHistory` / `remediation.recordHistory` / `remediation.deleteHistory` 仅做 user-triggered app-local remediation history metadata；默认不发 provider 请求、不联网、不写 skill/agent config、不改 triage、不创建或回滚 snapshot、不执行脚本、不读 credentials、不持久化 raw prompt/response/trace、不云同步、不发 telemetry。

**Closeout status**: completed with explicit Computer Use/window blocker. V2.60 integrates `remediation.listHistory` / `remediation.recordHistory` / `remediation.deleteHistory`, app-local redacted remediation history metadata, native Analysis remediation history UI, protocol fixtures, focused Rust/Swift tests, full service tests, `pnpm check:macos`, fixture screenshot inspection, and real local launch validation. Real local app launch against the current bundle succeeded, but AX reported 0 app windows, Computer Use returned `cgWindowNotFound` for the absolute app path and `timeoutReached` for the app name; a direct `open` retry spawned a second same-bundle process while still reporting 0 AX windows. Both validation processes were cleaned up. No real-local screenshot was committed.

### V2.63 Verification Checklist（completed）

1. Focused Rust/protocol checks: `knowledge.buildLocalSkillMap` returns deterministic summary, nodes, edges, clusters, coverage rows, gap/blocker notes, evidence refs, prompt metadata, and safety flags from existing catalog/knowledge/similar/taxonomy/conflict/task/risk evidence only.
2. Focused Swift/model checks: Local Skill Map records decode and render map summary, node/edge/cluster/coverage rows, gap/blocker notes, evidence refs, and no-write/no-provider safety flags without exposing raw prompt/raw response/raw trace/secrets/unredacted paths.
3. Full gate: `pnpm check:macos`.
4. Real local launch (`./script/build_and_run.sh run` or `pnpm dev:macos`) and current `dist/SkillsCopilot.app` Computer Use / AX attempt; exercise the Local Skill Map UI/service flow against real local app data, and record any window/tool blocker explicitly.
5. Fixture screenshot inspection if screenshots are produced; real-local screenshots must not be committed when live UI exposes local paths.
6. `pnpm check:privacy` before handoff.
7. 复核 V2.63 口径：Local Skill Map remains user-triggered, deterministic/read-only, derived from existing catalog/knowledge/similar/taxonomy/conflict/task/risk evidence, with no new source of truth, no default map artifact persistence, no skill/config writes, no snapshot creation/rollback, no triage mutation, no scripts, no default provider request, no raw prompt/response/trace/secret persistence, no cloud sync, and no telemetry.

**Closeout status**: completed on 2026-06-12. Focused Rust/protocol tests, full service tests, service protocol fixture decode, supported-method dispatch coverage, focused Swift model/store tests, full `swift test --package-path apps/macos`, `pnpm check:macos`, `pnpm check:privacy`, and `git diff --check` passed. Real local Computer Use reached single-skill Analysis, displayed Local Skill Map, clicked `Build Map`, and rendered live results with nodes, edges, clusters, evidence, and safety sections; no live screenshot was committed. A later coordinator exact-path attach hit `cgWindowNotFound` / `remoteConnection` with duplicate same-bundle app processes present, recorded as a tool/window-layer retry blocker.
