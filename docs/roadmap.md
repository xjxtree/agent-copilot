...
| V2.33 | Safe batch actions | 已完成：仅对 verified writable agent/roots 支持 preview-first 批量 enable/disable；预览返回受影响技能、不可写跳过项与原因、snapshot/rollback 计划；Pi/Hermes/OpenClaw 保持 read-only。 |
| V2.34 | Cross-agent comparison view | 进行中：横向比较同名/相似 skills 在 Claude/Codex/opencode/Pi/Hermes/OpenClaw 中的状态、来源、风险和差异。 |
