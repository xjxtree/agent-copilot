...
>
> 真实本机 app 验证在 2026-06-13 对 V2.70 重跑：multi-agent V2.70 review、Rust cleanup/protocol checks、Swift build/tests、native layout checks、screenshot artifact verification、no-capture fixture smoke、`pnpm check:privacy` 与 `git diff --check` 已通过；full `pnpm check:macos` 通过 build/test/service 阶段后，在 fixture capture 阶段以 `locked-session: macOS session is locked; refusing to create screenshot evidence` fail-closed。Computer Use 返回 `timeoutReached`，`ioreg` 报告 `CGSSessionScreenIsLocked=Yes`，direct capture exit 6 with `locked-session`。不提交 fresh V2.70 screenshot；真实本机 Computer Use 仍需在解锁会话中补验。若被锁屏、AX/window resolution、Screen Recording permission 或工具层问题阻塞，必须记录 blocker，不能用 smoke 截图替代真实交互验证。
...
| V2.71 | Guided Cleanup safe-action deep links | planned：guided cleanup step 应能跳转到现有安全入口，如 impact preview、batch review、safe batch preview、detail filters 和 report/export review；实际 enable/disable/edit/remediation 继续走 preview-first / explicit-confirm。 |
...
2. V2.71：让 Guided Cleanup 连接到既有安全预览入口，形成可执行但不隐藏 apply 的复查路径。
3. V2.72：把 AX 窗口解析失败、fixture smoke 与 real local validation 的证据口径继续写入脚本和文档，扩展 V2.69 已加入的锁屏/黑屏拒绝，避免把无效截图误判为 UI 验证通过。
...
Closeout status: completed; V2.71 should focus on Guided Cleanup safe-action deep links without hiding apply or bypassing preview/confirm flows.
