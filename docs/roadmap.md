...
>
> 真实本机 app 验证在 2026-06-13 对 V2.68 重跑：multi-agent V2.68 review、focused Rust checks、Swift tests、native layout checks、`pnpm check:macos`、`pnpm check:privacy`、`git diff --check` 与 fixture screenshot inspection 已通过；fixture macOS smoke 能启动并捕获当前 `dist/SkillsCopilot.app` 窗口。真实本机验证中当前 bundle 进程可启动，CG window metadata 找到 app 窗口，但 macOS session locked (`CGSSessionScreenIsLocked=Yes`)，Computer Use 超时，final direct capture 为黑图；作为 V2.68 locked-session/window-capture blocker 记录。真实本机截图未提交。后续用户可见、UI 或 service protocol 变更仍需重跑，且不能用 smoke 截图替代真实交互验证。
...
| V2.69 | Privacy / Screenshot Mode + 本地化收束 | planned：增加截图/演示模式、路径脱敏、长路径折叠与 reveal-on-demand，统一中英文混排标签；真实本机 UI 证据应能在不泄露本地路径的前提下交付。 |
...
1. V2.69：提供 privacy/screenshot mode，解决真实本机截图会暴露本地路径的问题，并同步清理中英文混排、过长路径和过密标签。
2. V2.70：拆分 Swift 与 Rust 大文件，把 task、knowledge、cleanup、provider、session、lifecycle 等功能边界落到模块结构里，降低后续版本修改风险。
3. V2.71：让 Guided Cleanup 连接到既有安全预览入口，形成可执行但不隐藏 apply 的复查路径。
4. V2.72：把锁屏、黑屏、AX 窗口解析失败、fixture smoke 与 real local validation 的证据口径写入脚本和文档，避免把无效截图误判为 UI 验证通过。
