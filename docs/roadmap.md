...
>
> 最新真实本机 app 验证在 2026-06-15 对 V2.77 收口：current `dist/SkillsCopilot.app` 以 exact bundle path/PID targeting 启动，unlocked Computer Use 验证 PID `34909`、`skills-copilot.main-window`、`skills-copilot.validation-workbench`、`skills-copilot.validation-workbench.summary`、`skills-copilot.validation-workbench.evidence-standards` 与 canonical blocker rows，并记录 app-window evidence at `docs/ui-artifacts/v2.77-validation-workbench/completed.png`。后续 user-visible UI 版本（尤其 V2.79/V2.80）仍需新的 unlocked real-local Computer Use 验证；若被锁屏、AX/window resolution、Screen Recording permission 或工具层问题阻塞，必须记录 canonical blocker，不能用 smoke 截图替代真实交互验证。
...
### V2.73-V2.77 Closeout and V2.78-V2.83 版本规划

2026-06-15 real-local audit 记录在 [`real-local-app-audit-2026-06-15.md`](./real-local-app-audit-2026-06-15.md)。V2.73 已完成 Task / remediation performance and timeout recovery；V2.74 已完成 Real-local launch and window targeting stability；V2.75 已完成 Task input and input-method resilience；V2.76 已完成 Progressive Cockpit feedback；V2.77 已完成 Real-local validation workbench。Minimax-m3 / GLM-5.1 review triage findings are assigned into the normal version sequence below：
...
| V2.78 | Protocol / validation gate parity | Planned: synchronize service-protocol docs with all supported methods, add docs/method drift verification, align CI with local Swift/docs/validation gates, resolve V2.46-V2.64 verification-history governance, and tidy planning/version wording where needed | No protocol rename, payload expansion, public release automation, provider/write/script/cloud/telemetry scope |
| V2.79 | Privacy fixture and evidence-surface localization sweep | Planned: replace literal local host-port fixtures, extend privacy scanning for host-port fingerprints, and apply path redaction/collapse/reveal plus Chinese localization across Guided Cleanup, Skill Map, Review, Task Cockpit, and nested evidence cards | No credential reads, no network behavior change, no scanner/catalog fact mutation |
| V2.80 | Detail navigation and visual density polish | Planned: reset or intentionally restore detail scroll per Work surface; improve dense evidence-card hierarchy, long-list folding, summary rows, and two-column responsive behavior | No new service method, provider default call, write/apply path, or hidden task state |
| V2.81 | Swift service IPC cancellation cleanup | Planned: add cancellation/timeout cleanup around short-lived stdio sidecar calls, including child-process termination and safe pipe draining | Keep current JSON service protocol; no default daemon/socket redesign |
| V2.82 | Test isolation and core model test floor | Planned: isolate provider tests that mutate environment variables and add core model serde/stability tests | Test hygiene only; no provider credential persistence or model schema change unless a real bug is found |
| V2.83 | Continued module splitting | Planned: continue V2.70-style extraction for large Rust/Swift service, view, store, and test files by existing domains | Refactor-only; no new methods, UI surfaces, or product semantics |

V2.78-V2.83 remain planned. They become completed only after code, tests, docs, and required validation evidence prove the behavior. V2.79 and V2.80 specifically require fresh unlocked real-local Computer Use validation because they affect user-visible evidence surfaces.
