...
> Status: V2.63 Local Skill Map is complete in the current docs/protocol handoff. V2.1 through V2.63 are the synchronized baseline; V2.63 adds user-triggered, deterministic, read-only `knowledge.buildLocalSkillMap` over existing catalog/knowledge/similar/taxonomy/conflict/task/risk evidence with no new source of truth or default persistence.
...
**Goal**: keep the completed V2.26-V2.63 management/analysis/provider/prompt-safety/task-routing/benchmark/regression/trace/accuracy/cross-agent readiness/stale-drift/remediation/history/session-review/local-skill-map baseline stable, then move into provider observability. Users should understand why a finding exists, where a skill came from, whether a conflict is same-agent or cross-agent, which issues are already reviewed, whether a real task can be routed to the right skill/agent, whether an actual agent session used the expected skill with acceptable quality and risk, and how local skills connect across source, capability, similarity, conflict, task coverage, and risk evidence.
...
| P1 | V2.64-V2.67 Observability / cockpit / lifecycle / guided cleanup | Planned | Keep V2.63 Local Skill Map stable, then add provider observability, Task-first Cockpit, Skill Lifecycle Timeline, and Guided Cleanup Flow on top of completed session review and local map evidence | Users can inspect provider usage/cost/failures, work from a task-centered cockpit, see skill change history, and clean up issues through safe guided steps |
