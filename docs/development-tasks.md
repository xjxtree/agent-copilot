...
> Status: V2.77 Real-local validation workbench is complete. V2.1 through V2.77 are the synchronized completed baseline; V2.78 Protocol / validation gate parity is the next planned validation slice.
...
- Real local Computer Use baseline: latest completed validation is V2.77 on 2026-06-15. The current `dist/SkillsCopilot.app` was rebuilt and launched with exact bundle path/PID targeting, unlocked Computer Use targeted the exact app path, matched PID `34909` and `skills-copilot.main-window`, opened the Work surface `验证工作台`, verified `skills-copilot.validation-workbench`, `skills-copilot.validation-workbench.summary`, `skills-copilot.validation-workbench.evidence-standards`, canonical blocker rows, and recorded app-window evidence at `docs/ui-artifacts/v2.77-validation-workbench/completed.png`. Future user-visible UI changes, including V2.79 and V2.80, need fresh unlocked Computer Use validation; if blocked, record one canonical blocker code and do not commit invalid real-local screenshots.
- Quality gate for code/UI/protocol work: `pnpm check:macos`; add focused Rust/Swift tests when touching shared behavior.
- 2026-06-15 review triage: the verified Minimax/GLM findings are assigned into V2.78-V2.83. V2.78 owns protocol/docs/gate parity, V2.79 owns privacy fixture and evidence-surface localization, V2.80 owns detail navigation/visual density, V2.81 owns Swift IPC cancellation cleanup, V2.82 owns provider-test env isolation plus core model tests, and V2.83 owns continued module splitting. None of these versions may add provider default calls, write/apply paths, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, hidden task state, or public distribution automation.
...
V2.77 closeout evidence lives in [`v2.77-verification-checklist.md`](./v2.77-verification-checklist.md) and is guarded by `pnpm verify:v2.77-docs`. V2.77 is completed with a read-only validation workbench, canonical blocker explanations, stable `skills-copilot.validation-workbench`, PID `34909` real-local Computer Use evidence, screenshot evidence at `docs/ui-artifacts/v2.77-validation-workbench/completed.png`, and unchanged no-provider/write/apply/script/credential/cloud/telemetry semantics. V2.78-V2.83 remain planned.

## Post-V2.77 Version Plan

The 2026-06-15 Minimax-m3 and GLM-5.1 review findings are assigned into the regular version sequence below. Do not mark any version completed until code/docs, focused tests, shared gates, and real-local validation requirements are satisfied where applicable.

| Version | Priority | Work area | Planned scope | Completion signal |
| --- | --- | --- | --- | --- |
| V2.78 | P0 | Protocol / validation gate parity | Keep `docs/service-protocol.md` synchronized with all 88 `SUPPORTED_METHODS`; add a lightweight docs verifier; wire V2.73-V2.77 docs verifiers, Swift tests, validation blocker verification, and screenshot artifact verification into CI or a documented CI-equivalent gate; resolve V2.46-V2.64 verification-history governance without inventing missing evidence; refresh planning/version wording as needed | Method-table drift is caught automatically, CI/local gate expectations are aligned, and historical verification docs explain V2.46-V2.64 consistently |
| V2.79 | P0 | Privacy fixture and evidence-surface localization sweep | Replace literal local host-port fixtures such as `localhost:8317`; extend privacy scanning for non-allowlisted `localhost:PORT` / `127.0.0.1:PORT`; apply path redaction/collapse/reveal and Chinese localization consistently across Guided Cleanup, Local Skill Map, Review, Task Cockpit, and nested evidence cards | Privacy check catches the previous host-port fingerprint, and unlocked real-local screenshots can be inspected without exposing local paths or mixed-language primary workflows |
| V2.80 | P1 | Detail navigation and visual density polish | Reset or intentionally restore detail scroll per Work surface; improve dense evidence-card hierarchy, long-list folding, summary rows, and two-column responsive behavior for real catalog data | Switching major surfaces lands at a coherent page start, and real-catalog evidence remains scannable without losing safety metadata |
| V2.81 | P1 | Swift service IPC cancellation cleanup | Keep the short-lived stdio sidecar shape, but add cancellation/timeout cleanup around `ServiceClient.runService`, including child-process termination and safe pipe draining | Swift tests cover cancelled/failed service calls without leaked child processes; no daemon/socket is introduced by default |
| V2.82 | P1 | Test isolation and core model test floor | Wrap provider-related `std::env::set_var` / `remove_var` tests in RAII cleanup and serialization; add focused serialization/defaulting tests for `AgentId`, `Scope`, and core skill identity structs | Provider tests remain deterministic under normal parallel cargo test runs, and `cargo test -p skills-copilot-core` covers stable wire/model assumptions |
| V2.83 | P2 | Continued module splitting | Continue V2.70-style extraction for `crates/service/src/lib.rs`, `DetailView.swift`, and large store/test files by existing domains, without changing protocol names or payload semantics | Smaller modules land behind existing focused tests and `pnpm check:macos`; refactor does not add new service methods or UI surfaces |
...
12. Next consolidation: V2.78-V2.83 are planned as one version line: protocol/validation gate parity, privacy fixture and evidence-surface localization, detail navigation/visual density, Swift IPC cancellation cleanup, test isolation/core model tests, and continued module splitting. Preserve the V2.77 validation workbench evidence baseline before adding or changing user-visible validation surfaces.
...
| P0 | V2.78 Protocol / validation gate parity | Planned | Synchronize service-protocol docs with all supported methods, add method/docs drift verification, align CI with local docs/Swift/validation gates, and document V2.46-V2.64 verification history without invented evidence | Future protocol/docs drift is caught automatically, and maintainers have one reliable gate story |
| P0 | V2.79 Privacy fixture and evidence-surface localization sweep | Planned | Replace literal local host-port fixtures, extend privacy scanning for host-port fingerprints, and apply path redaction/collapse/reveal plus Chinese localization across evidence surfaces | Real-local screenshots can be visually inspected without exposing local paths, and Chinese users do not hit mixed-language primary workflows |
| P1 | V2.80 Detail navigation and visual density polish | Planned | Reset or intentionally restore detail scroll per Work surface; improve dense evidence-card hierarchy, long-list folding, summary rows, and two-column responsive behavior | Switching major surfaces lands at a coherent page start, and real-catalog evidence remains scannable without losing safety metadata |
| P1 | V2.81 Swift service IPC cancellation cleanup | Planned | Add cancellation/timeout cleanup around the short-lived stdio sidecar without introducing a background daemon by default | Cancelled/failed service calls do not leak child processes or pipe handles |
| P1 | V2.82 Test isolation and core model test floor | Planned | Isolate provider tests that mutate environment variables and add core model serde/stability tests | Parallel tests remain deterministic and core identity/scope assumptions are covered |
| P2 | V2.83 Continued module splitting | Planned | Continue splitting large Rust/Swift service, view, store, and test files by existing domains | Follow-up work lands in smaller modules without changing service protocol or product semantics |
...
- If the task is AI provider foundation, prompt safety, AI quality/readiness/routing, task benchmark/regression, trace analysis, knowledge index, remediation, Agent Session Skill Review, Local Skill Map, provider observability, task cockpit, skill lifecycle, guided cleanup, cockpit-first IA, screenshot/privacy-safe UI presentation, completed Swift/Rust feature modularization, guided-cleanup safe-action links, validation harness hardening, completed Task/remediation timeout recovery, completed launch/window targeting stability, completed task input resilience, completed progressive Cockpit feedback, or completed real-local validation workbench, use V2.41-V2.77.
- If the task is post-V2.71 product consolidation or validation harness hardening, use V2.72.
- If the task is protocol/docs/gate parity after V2.77, use V2.78 and keep status planned/in-progress until code, tests, docs, and validation evidence exist.
- If the task is privacy fixture hardening or evidence-surface privacy/localization sweep after V2.77, use V2.79 and keep status planned/in-progress until code, tests, docs, and unlocked real-local validation evidence exist.
...
- If the task is continued Swift/Rust module splitting after V2.70, use V2.83.
