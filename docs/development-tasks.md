...
> Status: V2.79 Privacy fixture and evidence-surface localization sweep is complete. V2.1 through V2.79 are the synchronized completed baseline; V2.80 Detail navigation and visual density polish is the next planned validation slice.
...
- Real local Computer Use baseline: latest completed UI validation is V2.79 on 2026-06-15. The current `dist/SkillsCopilot.app` was rebuilt and launched with exact bundle path/PID targeting, unlocked Computer Use targeted the exact app path, matched PID `68064` and `skills-copilot.main-window`, verified localized Work surfaces, opened `验证工作台`, verified `skills-copilot.validation-workbench`, `skills-copilot.validation-workbench.summary`, `skills-copilot.validation-workbench.evidence-standards`, canonical blocker rows, and recorded app-window evidence at `docs/ui-artifacts/v2.79-privacy-localization/completed.png`. V2.78 did not change native user-visible UI, so no fresh Computer Use screenshot was required. Future user-visible UI changes, including V2.80, need fresh unlocked Computer Use validation; if blocked, record one canonical blocker code and do not commit invalid real-local screenshots.
- Quality gate for code/UI/protocol work: `pnpm check:macos`; add focused Rust/Swift tests when touching shared behavior.
- 2026-06-15 review triage: the verified Minimax/GLM findings assigned to V2.79 are complete; the remaining findings are assigned into V2.80-V2.83. V2.80 owns detail navigation/visual density, V2.81 owns Swift IPC cancellation cleanup, V2.82 owns provider-test env isolation plus core model tests, and V2.83 owns continued module splitting. None of these versions may add provider default calls, write/apply paths, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, hidden task state, or public distribution automation.
...
- If the task is AI provider foundation, prompt safety, AI quality/readiness/routing, task benchmark/regression, trace analysis, knowledge index, remediation, Agent Session Skill Review, Local Skill Map, provider observability, task cockpit, skill lifecycle, guided cleanup, cockpit-first IA, screenshot/privacy-safe UI presentation, completed Swift/Rust feature modularization, guided-cleanup safe-action links, validation harness hardening, completed Task/remediation timeout recovery, completed launch/window targeting stability, completed task input resilience, completed progressive Cockpit feedback, completed real-local validation workbench, completed protocol/docs/gate parity, or completed privacy fixture/evidence localization, use V2.41-V2.79.
