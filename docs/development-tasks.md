...
> Status: V2.61 AI Analysis UX / Prompt Run History is completed in the current candidate. V2.1 through V2.61 are the synchronized baseline; V2.61 consolidates the native Analysis page, extends provider-backed AI analysis timeout to 10 minutes, and adds app-local redacted prompt run history.
...
3. Review whether an agent actually used the right skill. V2.62 should center on imported/pasted agent sessions and answer: expected skill, observed skill, miss/wrong-pick/ambiguity, evidence, and safe next step.
...
| V2.62 | Agent Session Skill Review | Planned | Create local read-only review sessions from imported traces, pasted agent transcripts, or future explicitly selected local agent sessions; judge skill discovery/selection/use, wrong-pick, miss, ambiguity, duplicate-skill interference, and safe next steps. App AI prompt runs are auxiliary evidence only |
...
**Goal**: keep the completed V2.26-V2.51 management/analysis/provider/prompt-safety/task-routing/benchmark/regression/trace/accuracy/cross-agent readiness/stale-drift baseline stable, then move into AI-native session review, local skill mapping, and provider observability. Users should understand why a finding exists, where a skill came from, whether a conflict is same-agent or cross-agent, which issues are already reviewed, and whether a real task can be routed to the right skill/agent with acceptable quality and risk over repeatable local benchmarks, saved baselines, redacted local trace evidence, accuracy dashboards, cross-agent readiness comparisons, and stale/drift signals.
...
11. V2.41-V2.67 AI-native skill review and observability: V2.41 provider foundation, V2.42 prompt safety, V2.43 skill quality, V2.44 task readiness, V2.45 routing confidence, V2.46 task benchmark set, V2.47 routing regression detection, V2.48 trace import, V2.49 routing accuracy dashboard, V2.50 cross-agent task readiness, V2.51 stale/drift detection, V2.52 local knowledge index, V2.53 similar skill grouping, V2.54 capability taxonomy, V2.55 workspace readiness, V2.56 remediation planner, V2.57 fix preview drafts, V2.58 impact preview, V2.59 batch review, V2.60 remediation history, and V2.61 prompt run history are completed. The remaining planned items are V2.62 Agent Session Skill Review, V2.63 Local Skill Map, V2.64 provider observability, V2.65 Task-first Cockpit, V2.66 Skill Lifecycle Timeline, and V2.67 Guided Cleanup Flow.
...
| P1 | V2.62-V2.67 Skill review / map / observability / cockpit / lifecycle / guided cleanup | Planned | Add Agent Session Skill Review, Local Skill Map, provider observability, Task-first Cockpit, Skill Lifecycle Timeline, and Guided Cleanup Flow | Users can review real agent sessions, understand the local skill graph, inspect provider usage/cost/failures, work from a task-centered cockpit, see skill change history, and clean up issues through safe guided steps |
