...
> Status: current planning and execution queue as of 2026-06-09. V2.1 through V2.10 are closed on the main line. Current phase is V2.11 Adapter Capability Matrix. Near-term priority is comprehensive agent adapter support for opencode writable support, Pi, Hermes, and OpenClaw evidence/implementation in the macOS app.
...
- Current priority: complete V2.11 Adapter Capability Matrix, then close the remaining agent adapter evidence and implementation gaps in the macOS app.
...
| V2.11 | Adapter Capability Matrix | In progress | Service protocol and macOS UI expose scan/toggle/install status and blockers for Claude Code, Codex, opencode, Pi, Hermes, and OpenClaw |
| V2.12 | opencode writable support | Planned next | Disposable local evidence verifies `permission.skill` writes, then guarded toggle/install is implemented or blocker remains explicit |
| V2.13 | Pi adapter support | Planned | Disposable local round-trip verifies roots/schema/toggle/rollback, then read/write scope is implemented as evidence permits |
...
1. V2.11 Adapter Capability Matrix.
2. V2.12 opencode writable evidence, toggle/install design, and guarded implementation if evidence supports it.
3. V2.13 Pi disposable local round-trip and adapter implementation plan.
4. V2.14 Hermes maintainer-confirmed spec and adapter plan.
5. V2.15 OpenClaw maintainer-confirmed spec and adapter plan.
...
| P0 | V2.11 Adapter Capability Matrix | First service/UI slice implemented; validation pending | Run focused protocol/UI checks, then use the matrix as the gate for future Pi/opencode/Hermes/OpenClaw work | macOS UI shows precise scan/toggle/install status and blockers for all six agents |
