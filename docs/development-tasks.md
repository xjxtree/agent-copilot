...
> Status: V2.72 Validation harness hardening is complete. V2.1 through V2.72 are the synchronized completed baseline; validation evidence now has a canonical blocker taxonomy and fixture/real screenshot matrix.
...
- Real local Computer Use baseline: latest final validation attempt is V2.72 on 2026-06-13. Validation blocker classifier checks, screenshot artifact verification, no-capture fixture smoke, `pnpm check:privacy`, and `git diff --check` passed. Capture-window fixture smoke and full `pnpm check:macos` fail closed at fixture capture with canonical `locked-session`; Computer Use still returns `timeoutReached`. Real-local Computer Use and visual screenshot review still must be rerun when the macOS session is unlocked; if blocked, record one of the canonical blocker codes and do not commit invalid real-local screenshots.
...
V2.65-V2.72 completed those next product directions: a task-first cockpit that groups readiness/routing/session-review evidence by user task, a skill lifecycle view that shows new/stale/duplicate/risky skills over time, a guided cleanup flow that keeps write paths preview-first and explicit-confirm only, a cockpit-first IA that makes those surfaces reachable without hunting through one dense Analysis panel, screenshot-safe path display for validation evidence, smaller Swift/Rust feature modules for safer follow-up work, safe guided-cleanup links into existing review/preview surfaces, and a hardened validation harness that rejects invalid UI evidence.
...
| Version | Work area | Planned scope | Non-goals |
| --- | --- | --- | --- |
| V2.73 | Task / remediation performance and timeout recovery | Profile and optimize task readiness/routing/cross-agent/remediation/batch review paths on real catalog data; add bounded service timeouts, fallback result rows, cancel/retry UI, and tests that prevent indefinite Cockpit loading | No provider default calls, no write/apply path, no safety-boundary expansion |
| V2.74 | Real-local launch and window targeting stability | Stabilize dev launch against duplicate bundle ids and stale worktree bundles; make window creation/restoration and exact path/PID Computer Use targeting diagnosable | No signing/notarization/public distribution |
| V2.75 | Task input and input-method resilience | Validate Chinese input method, paste, multiline task entry, focus, shortcuts, and automation-safe entry; preserve task text exactly before service calls | No raw prompt persistence or cloud sync |
| V2.76 | Progressive Cockpit feedback | Show readiness/routing/remediation/provider/session stages, partial rows, elapsed time, timeout/fallback explanations, and clear blocked states | No new analysis surface by default |
| V2.77 | Real-local validation workbench | Add a focused preflight/status surface or runbook-driven workflow for lock state, Screen Recording permission, window-not-found, stale bundle, duplicate bundle id, and invalid screenshot blockers | No substitute for unlocked visual review |
...
| P1 | V2.65-V2.72 Cockpit / lifecycle / guided cleanup / IA / privacy / modularization / safe links / validation | V2.65 completed; V2.66 completed; V2.67 completed; V2.68 completed; V2.69 completed; V2.70 completed; V2.71 completed; V2.72 completed | Keep Task-first Cockpit, Skill Lifecycle Timeline, Guided Cleanup Flow, cockpit-first IA, screenshot privacy, module boundaries, safe guided-cleanup navigation, and validation blocker taxonomy stable | Users can work from a task-centered cockpit, see skill change history, clean up issues through safe guided steps, reach major evidence surfaces, produce screenshot evidence without leaking local paths, reject invalid screenshot evidence, and benefit from safer follow-up maintenance |
...
| P0 | V2.73 Task / remediation performance and timeout recovery | Planned | Fix real-catalog task/remediation service calls that can leave Task Cockpit in `Preparing...`; add timeout/fallback/cancel/retry and focused real-data performance checks | Users can trust the task-centered cockpit on real local catalogs instead of seeing an indefinite loading state |
| P0 | V2.74 Real-local launch and window targeting stability | Planned | Make current bundle path/PID launch, duplicate bundle cleanup, window restoration, AX/Computer Use targeting, and lock-screen diagnosis repeatable | Real local validation can target the latest app without stale-bundle or no-window ambiguity |
| P1 | V2.75 Task input and input-method resilience | Planned | Preserve user task text across Chinese input method, paste, multiline entry, focus changes, and automation-safe input paths | Users can enter tasks reliably before readiness/routing analysis |
| P1 | V2.76 Progressive Cockpit feedback | Planned | Replace single `Preparing...` feedback with staged readiness/routing/remediation/provider/session progress, partial results, elapsed time, and fallback rows | Users can tell whether the app is working, blocked, or safely degraded |
| P1 | V2.77 Real-local validation workbench | Planned | Make lock/window/permission/stale-bundle/screenshot blocker state visible and actionable for real-local validation | Invalid screenshots still remain blockers, not accepted evidence |
| P0 | V2.78 Evidence-surface privacy and localization sweep | Planned | Apply path redaction/collapse/reveal and Chinese localization consistently across Guided Cleanup, Local Skill Map, Review, Task Cockpit, and nested evidence cards | Real-local screenshots can be visually inspected without exposing local paths, and Chinese users do not hit mixed-language primary workflows |
| P1 | V2.79 Detail navigation and visual density polish | Planned | Reset or intentionally restore detail scroll per Work surface; improve dense evidence-card hierarchy, long-list folding, summary rows, and two-column responsive behavior | Switching major surfaces lands at a coherent page start, and real-catalog evidence remains scannable without losing safety metadata |
