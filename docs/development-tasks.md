...
> Status: V2.54 Capability Taxonomy is completed and synchronized on `main`; V2.1 through V2.54 are synchronized baseline; V2.55 workspace readiness is the next roadmap line and is still planned around `workspace.checkReadiness`.
...
| V2.55 | Workspace Readiness Check | Planned | `workspace.checkReadiness` should evaluate whether the current project has the right skills enabled and scoped per agent for expected work, using local catalog/taxonomy/readiness evidence only |
