...
- Latest screenshot was regenerated by `pnpm check:macos` during fixture Smoke App Run. The smoke script used the `SkillsCopilot` window id and Quartz window capture, so it contains the complete app window only.
