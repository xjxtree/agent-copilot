...
- `RoutingRegression` 结果记录对比 baseline 与当前基线的命中率、漏选率、错选率、ambiguity 变化；默认在 app-local 数据保存，不触发 provider 请求。可选 provider explanation 仍走 V2.42 流程（preview / redaction / confirm / copy-only）。
