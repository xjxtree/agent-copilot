...
- `ImpactPreview` / `ImpactPreviewItem`（V2.58 planned）：local-only, user-triggered deterministic impact preview before enable/disable/edit/remediation actions. Response shape: `{ generated_by, catalog_available, filters, summary, impact_items, risk_delta_rows, rollback_plan_rows, gap_notes, blocker_notes, evidence_references, prompt_request, safety_flags }`; preview items describe impacted tasks, agents, skills, writable capability matches, filtered/blocked write paths, snapshot/rollback planning, and evidence refs. It must not apply actions, write skill files, mutate agent config, create/rollback snapshots, mutate triage, execute scripts, read credentials, persist raw prompt/response/trace, sync cloud, emit telemetry, or send default provider traffic. Any provider wording remains V2.42 gated copy-only.
