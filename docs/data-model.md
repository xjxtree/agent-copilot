...
- `CapabilityTaxonomy` / `WorkspaceReadiness`（V2.54-V2.55 planned）：intended derived grouping and readiness views over existing catalog evidence; planned entry points are `knowledge.buildCapabilityTaxonomy` and `workspace.checkReadiness`; local, user-triggered, deterministic, read-only, and not yet implemented or validated in this branch.
