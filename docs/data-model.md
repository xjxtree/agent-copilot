...
- `WorkspaceReadiness`（V2.55 planned）：future derived readiness view over existing catalog / taxonomy / task readiness evidence; planned entry point is `workspace.checkReadiness`; local, user-triggered, deterministic, and read-only by default. Planned response shape is expected to stay in the existing read-only style: `{ generated_by, catalog_available, filters, summary, readiness_rows, gap_notes, blocker_notes, evidence_references, prompt_request, safety_flags }`, with row fields for agent/workspace/scope/risk/readiness signals and no persisted readiness artifact.
