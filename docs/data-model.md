...
- `PreviewDraft` / `PreviewDraftGroup`（V2.57 planned）：local-only, user-triggered draft suggestions for frontmatter, description, permissions, dependency, and policy. Response shape: `{ generated_by, catalog_available, filters, summary, draft_groups, draft_rows, gap_notes, blocker_notes, evidence_references, prompt_request, safety_flags }`; draft rows are copy/edit-ready suggestions with no apply/write path, no persisted raw prompt/response/trace by default, and any provider wording remains V2.42 gated copy-only.
- `ReviewSession` / `RemediationHistory`（V2.58-V2.61 future）：local review state, actions considered, decisions, reopened issues, impact notes, and summary. AI suggestions remain untrusted and cannot directly mutate skill files or agent config.
