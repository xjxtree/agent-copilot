...
## V2.41-V2.71 AI-native skill review models

The AI-native line introduces analysis models incrementally and evidence-first. V2.41 adds app-data provider profile metadata and Keychain credential references, V2.61 adds redacted prompt run history, V2.62 adds app-local redacted Agent Session Skill Review metadata, V2.63 adds a derived Local Skill Map response, V2.64 adds a completed Provider Observability view, V2.65 adds a completed Task-first Cockpit response, V2.66 adds a completed Skill Lifecycle Timeline response, V2.67 adds completed Guided Cleanup Flow response plus app-local redacted step metadata, and V2.71 adds navigation-only guided cleanup safe-action link metadata.
...
- `PromptRunRecord`（V2.61 completed）：app-local provider-backed prompt run history stored as `prompt-runs.json`. Records contain run id, preview/confirmation id, action/request kind, optional analysis kind, scope, selected instance ids, redacted task text, provider/model/destination metadata, status/error/duration/token/cost metadata, extracted copy-only draft output, redaction summary, timestamps, and safety flags. Records are created only after explicit prompt preview + confirmation. They may hydrate the latest visible AI analysis result after app restart and reruns append new records. They must not contain API keys, raw prompts, raw provider response JSON, raw trace excerpts, unredacted local paths, or any write/script/config/snapshot/triage side effect.
...
- `BatchReviewSession` / `BatchReviewGroup` / `BatchReviewItem`（V2.59 completed）：local-only, user-triggered deterministic batch review state from `remediation.batchReview`. Response shape: `{ generated_by, catalog_available, filters, summary, review_groups, review_items, recommended_next_step_labels, gap_notes, blocker_notes, evidence_references, prompt_request, safety_flags }`; review groups/items organize candidate next steps by task, risk, rule, agent, and workspace with safe next-step labels and evidence refs. It does not apply actions, mutate triage, create or roll back snapshots, write skill files or agent config, execute scripts, read credentials, persist raw prompt/response/trace, sync cloud, emit telemetry, or send default provider traffic.
- `RemediationHistory` / `RemediationHistoryRecord`（V2.60 completed）：app-local remediation history metadata stored in `remediation-history.json` through explicit user action. `remediation.recordHistory` stores redacted title/decision/status/source refs, optional agent/workspace/task context, rule/risk tags, recurrence/reopened markers, readiness/routing improvement notes, blocker/gap notes, evidence refs, redaction summary, timestamps, and safety flags. `remediation.listHistory` returns filters, summary counts, history records, optional recurrence rows, blocker notes, and no-provider/no-write safety flags. `remediation.deleteHistory` deletes only that app-local metadata record. No operation writes skill files or agent config, mutates triage, creates or rolls back snapshots, executes scripts, reads credentials, persists raw prompt/response/trace, syncs cloud data, emits telemetry, or sends default provider traffic.
- `AgentSessionSkillReview`（V2.62 completed）：local read-only review session created by `session.reviewAgentSkillUse` from V2.48 imported traces, pasted/imported agent transcripts, or future explicitly selected local agent sessions; `session.listSkillReviews` lists records and `session.deleteSkillReview` deletes one metadata record. It stores only app-local `agent-session-reviews.json` redacted metadata/excerpt plus deterministic review output: review id/title/source kind, agent/task context, referenced trace import ids, expected skill refs/names, detected skill refs/names, outcome (`hit` / `miss` / `wrong_pick` / `ambiguous` / `unknown`), duplicate/similar-skill interference notes, safe next steps, evidence refs, redaction summary, timestamps, and safety flags. It must not persist raw transcript, raw prompt, raw response, secrets, credentials, unredacted local paths, skill file contents, or agent config contents. It cannot mutate skill files, agent config, triage, snapshots, cleanup records, or routing baselines; it cannot execute scripts, send provider requests, sync cloud data, or emit telemetry.
- `LocalSkillMap` / `LocalSkillMapNode` / `LocalSkillMapEdge` / `LocalSkillMapCluster`（V2.63 completed）：derived read-only graph/map produced by `knowledge.buildLocalSkillMap` over existing catalog evidence, V2.52 knowledge tags, V2.53 similar groups, V2.54 capability taxonomy, same-agent conflicts, cross-agent analysis, task readiness/routing/session-review context, stale/drift signals, source provenance, and risk context.
  - `knowledge.buildLocalSkillMap` response: `{ generated_by, catalog_available, filters, summary, nodes, edges, clusters, domains, risk_notes, gap_notes, blocker_notes, evidence_references, prompt_request, safety_flags }`。
  - `filters`: `{ agent, task, limit, node_limit, edge_limit, cluster_limit, candidate_instance_ids, include_task_context }`。
  ...
  - `nodes`: skill/source/capability/similar-group/conflict/task/risk nodes with stable ids, labels, node type, agent/scope/state/enabled where relevant, counts/scores, evidence refs, and safety flags. Nodes reference existing catalog/evidence ids instead of embedding raw skill bodies or unredacted paths.
  - `edges`: typed relationships such as `skill_agent`, `skill_source`, `skill_risk`, `skill_capability`, `similar_group_member`, `same_agent_conflict`, `cross_agent_analysis`, `task_readiness`, and `task_route_candidate`, each with source/target ids, weight, reason labels, evidence refs, and safety flags.
  - `clusters` / `domains`: capability-domain, similarity, conflict, and per-capability coverage summaries that help the UI group map regions without persisting a new graph artifact.
  - Safety boundary: user-triggered, deterministic, local-only, read-only, no new source of truth, no app-local map artifact/state persistence in V2.63, no provider request by default, no skill/config/snapshot/triage/script/credential mutation, no raw prompt/raw response/raw trace/secret/unredacted-path persistence, no cloud sync, and no telemetry.
- `ProviderObservabilityView`（V2.64 completed）：derived read-only UI/reporting response from `llm.providerObservability` over `ProviderCallMetadataMinimal` and V2.61 `PromptRunRecord` metadata.
  - Response shape: `{ generated_by, status, summary, call_rows, history_rows, grouping_rows, status_rows, budget_usage_hints, retention_recommendations, gap_notes, blocker_notes, evidence_references, prompt_metadata, safety_flags }`。
  - `summary` includes visible call/run counts, success/error/timeout counts, provider/model/destination coverage, estimated token/cost totals when available, latest call timestamps, and a human-readable status summary.
  - `call_rows` and `history_rows` are redacted app-local metadata rows only: provider/model/destination host, action/request kind, status/error category, duration, token/cost estimate, confirmation/redaction ids when available, and evidence refs to prompt run or call metadata records.
...
- `GuidedCleanupFlow` / `GuidedCleanupStep`（V2.67 completed）：guided cleanup response and app-local step metadata derived from existing local evidence only: findings, cleanup queue, similar groups, stale/drift, readiness/routing/task cockpit, lifecycle timeline, remediation plan/drafts/impact/batch review, adapter diagnostics, and source provenance.
  ...
  - `flow_steps`: stable ids, rank, step type, phase, status, issue/risk/task labels, affected skill/agent/workspace refs, recommended safe next-step labels, safe entry method, existing safe method when one exists, V2.71 `safe_action_deep_link`, explicit-confirmation flag, blocker/gap notes, evidence refs, side-effect flags, and no-write/no-provider safety flags. Rows reference existing evidence ids and must not embed raw skill bodies, raw prompt/response content, raw trace excerpts, secrets, credentials, or unredacted local paths.
  - `issue_groups` and `safe_next_actions` summarize grouped issues and safe review entry points without applying fixes. V2.71 `safe_next_actions` include `deep_link` metadata to existing safe entry points only; `can_apply` must remain false.
  - `GuidedCleanupSafeActionDeepLink`: `{ label, target, detail_section, method, trigger, preview_only, requires_confirmation, copy_only, can_apply, instance_ids, related_step_ids, evidence_refs, safety_flags }`. Allowed methods are existing read-only/preview-first entries only: cleanup list/detail, remediation plan/drafts/impact/batch review, skill lifecycle, task cockpit, safe batch preview panel context, and guided step metadata recording. It cannot represent direct apply/toggle/script/provider-confirm actions.
...
- AI output is explanation/suggestion data; it must not become an execution requester, config writer, snapshot actor, or hidden state mutation.
