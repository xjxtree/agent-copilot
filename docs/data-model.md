...
The AI-native line introduces analysis models incrementally and evidence-first. V2.41 adds app-data provider profile metadata and Keychain credential references, V2.61 adds redacted prompt run history, and V2.62 adds app-local redacted Agent Session Skill Review metadata. The remaining planned models cover Local Skill Map, Provider Observability, Task-first Cockpit, Skill Lifecycle Timeline, and Guided Cleanup Flow.
...
- `LocalSkillMap`（V2.63 future）：derived graph/map over existing catalog evidence, knowledge tags, similar groups, capability taxonomy, conflicts, task coverage, and risk context. It is read-only and should not create a new source of truth separate from catalog evidence.
