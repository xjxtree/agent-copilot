...
- `RemediationHistory`（V2.60 planned/in-progress）：local app-data remediation history state with redacted metadata only, including actions considered, decisions, reopened issues, impact notes, and summary. If the service exposes record/list/delete/clear, those operations stay user-triggered and local-only; AI suggestions remain untrusted and cannot directly mutate skill files or agent config.
