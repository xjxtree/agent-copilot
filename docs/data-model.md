...
- `BatchReviewSession` / `BatchReviewGroup` / `BatchReviewItem`（V2.59 planned）：local-only batch review state that groups candidate next steps by task, risk, rule, agent, and workspace; planned response shape should expose summary, review groups/items, safe next-step labels, gap notes, blocker notes, evidence refs, prompt metadata, and safety flags. It does not apply actions, mutate triage, create or roll back snapshots, write skill files or agent config, execute scripts, read credentials, persist raw prompt/response/trace, sync cloud, or emit telemetry.
