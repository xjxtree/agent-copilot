...
The AI-native line introduces analysis models incrementally and evidence-first. V2.41 adds app-data provider profile metadata and Keychain credential references, and V2.61 adds redacted prompt run history. The remaining planned models cover Agent Session Skill Review, Local Skill Map, Provider Observability, Task-first Cockpit, Skill Lifecycle Timeline, and Guided Cleanup Flow.
...
- `AgentSessionSkillReview`（V2.62 future）：local read-only review session created from V2.48 imported traces, pasted/imported agent transcripts, or future explicitly selected local agent sessions. It stores only redacted metadata/excerpt, detected/expected skill refs, outcome (`hit` / `miss` / `wrong_pick` / `ambiguous` / `unknown`), duplicate/similar-skill interference notes, evidence refs, optional copy-only AI explanation metadata, and safety flags. It must not persist raw transcript, raw prompt/response, credentials, skill file contents, or agent config contents, and it cannot directly mutate skill files or agent config.
