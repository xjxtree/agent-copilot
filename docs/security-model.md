...
### 2.2.1.1 Pi writable evidence harness（V2.36）

Pi settings mutation remains blocked in product write paths until the V2.37 guarded slice implements and verifies a minimal toggle path. V2.36 evidence harness has passed in disposable roots only. During the evidence phase, `pi` agent writes are treated as unsupported or read-only actions unless evidence confirms:
...
该门槛成立前，`config.toggleSkill` 等通道应返回 `read-only`/`unsupported` 风险提示，而非尝试直接写入 `~/.pi/agent/settings.json` 或 `.pi/settings.json`。

该证据项已完成；`V2.37` 可评估并实现 `pi` adapter 的受限 writable slice，任何回退路径必须支持回滚，且 install 继续 blocked。
