...
### 2.4.3.1.5 V2.51 Stale / Drift Detection（completed）

- `analysis.detectStaleDrift` 是用户触发的 read-only stale/drift 入口：围绕 fingerprint、mtime staleness、finding/conflict/analysis drift、source/root provenance、adapter diagnostics 和 readiness impact 做本地 deterministic 分析。
- 本方法默认不发起 provider 请求；核心证据必须先由本地 deterministic 路径生成：catalog fingerprint / metadata、rule findings、same-agent conflicts、cross-agent analysis、source provenance、adapter diagnostics、以及派生 readiness impact notes。
- 禁止在该方法路径执行：skill 文件写入、agent-config 写入、stale/drift artifact 写入、snapshot 创建/回滚、triage 变更、scripts 执行、credentials 读写。
- 同时禁止新建 raw path/prompt/response/raw trace 持久化；输出可包含 redacted 证据引用与可复查 IDs，不应包含 secret、raw prompt、raw response、raw trace 或未脱敏路径。
...
- V2.55 workspace readiness is implemented as `workspace.checkReadiness` and must not be inferred from V2.52 alone; it is local-only, user-triggered, deterministic, and read-only by default, with no provider requests by default, no skill/config/snapshot/triage/readiness-artifact writes, no scripts, no credentials, no raw prompt/response/trace persistence, and no cloud sync/telemetry. V2.53 similar grouping and V2.54 taxonomy are documented separately as read-only local slices; V2.56 remediation planning is implemented as a read-only local planner.

### 2.4.3.1.7 V2.53 Similar Skill Grouping（completed）

- `knowledge.groupSimilarSkills` is user-triggered, local-only, read-only, deterministic, and evidence-first. It groups same/similar/confusable skills using existing catalog evidence, V2.52 derived tags, and source/name/tool/rule/capability/risk overlaps.
- The method must not write skill files, agent config, grouping artifacts, snapshots, triage state, or raw trace content; it must not execute scripts; it must not read or write credentials.
...
- `remediation.plan` is user-triggered, local-only, deterministic, and read-only by default. It consumes findings, gaps, routing ambiguity, drift, task/workspace context, and candidate instance IDs to derive prioritized remediation rows.
- The method must not write skill files, agent config, remediation artifacts, snapshots, triage, or scripts; it must not read or write credentials; it must not default to provider or network usage.
...
- 仅对 verified writable config-toggle agent/roots 执行；Pi 仅在 guarded settings scope 内参与，Hermes 和 OpenClaw 是 install-only 且不参与 config-toggle batch；blocked 或能力不满足项归入跳过集合并给出具体跳过原因。
...
## V2.39/V2.96 OpenClaw scope security note (completed)

- OpenClaw deepening in V2.39 is workspace-scoped; V2.96 adds install-only
  support.
...
- Confirmed install roots: `~/.openclaw/skills` and `<workspace>/skills`
  only.
- `.agents` direct installs, config toggles, `skills.entries` writes, ClawHub,
  Git, update, verify, workshop, and network-backed operations remain blocked.
...
- These constraints reflect the completed V2.39 scanner and V2.96 install-only
