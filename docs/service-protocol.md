...
| V2.60 | `(planned) remediation.history` | Planned local app-data remediation history surface; target behavior is redacted metadata only, user-triggered read/list/delete/clear, and no provider/network/write/script/config paths |
...
V2.60 remediation history is documented as planned/in-progress only. The target service shape is local app-data history records with redacted metadata only, user-triggered record/list/delete/clear operations, and no provider/network/write/script/config paths. It remains a docs-level contract until the coordinator integrates the actual method surface.
