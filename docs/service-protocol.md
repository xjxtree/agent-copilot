...
## V2.41-V2.46 AI Provider Foundation, Prompt Safety, Quality, Readiness, Routing Confidence, And Task Benchmarks / V2.47+ planning

Current implementation status after V2.46:
...
- Not yet integrated: V2.47 regression detection、V2.48-V2.49 trace models，and full V2.69 provider observability UX over call metadata.

V2.47+ planning starts from the V2.46 completed protocol surface below.
...
| V2.47 | `(planned) task.detectRoutingRegression` | Compares latest benchmark outputs against a stored baseline and emits local regression signals |
| V2.48-V2.49 | `(planned) trace.importLocal`, `(planned) trace.analyzeRoutingAccuracy`, `(planned) routing.dashboard` | Local trace import with redaction; no raw credential/log persistence |
| V2.50-V2.55 | `(planned) task.compareAgentReadiness`, `(planned) knowledge.search`, `(planned) knowledge.groupSimilarSkills`, `(planned) workspace.checkReadiness` | Local index/readiness views; no default network |
...
V2.47 additive routing-regression surface (planned):

- Planned `task.detectRoutingRegression` request includes:
  - `agent`, `benchmark_ids`, optional `candidate_instance_ids`, optional `compare_to` (baseline snapshot id / time), optional `include_ai_commentary`.
- Planned response includes:
  - `regression_items`, `risk_summary`, `wrong_pick_delta`, `miss_delta`, `ambiguity_delta`, `evidence_references`, and `safety_flags`.
- Regression analysis is local and deterministic-first; optional AI explanation is only copy/display-only and must remain preview/redaction/confirmation-gated through existing V2.42 path.
