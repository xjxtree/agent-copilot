...
>
> V2.78 completed: protocol / validation gate parity is a docs/gate governance slice over the existing service protocol. `crates/service/src/lib.rs` `SUPPORTED_METHODS` is the source of truth for method names, currently 88 methods, and `pnpm verify:service-protocol-drift`, `pnpm verify:v2.78-docs`, and `pnpm verify:gate-parity` compare docs/gates against that list. This closeout does not rename protocol methods, expand payloads, add provider defaults, add write/apply paths, execute scripts, read credentials, sync cloud data, emit telemetry, or weaken real-local validation standards.
>
