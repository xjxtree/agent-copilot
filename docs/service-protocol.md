...
| V2.62 | `(planned) reviewSession.*` | Local redacted Agent Session Skill Review records created from imported traces, pasted agent transcripts, or future explicitly selected local agent sessions; reviews skill discovery/selection/use, misses, wrong picks, ambiguity, and duplicate-skill interference |
| V2.63 | `(planned) skillMap.*` | Local skill graph/map over existing catalog evidence: relationships, sources, capability domains, similar groups, conflicts, task coverage, and risk |
...
| V2.65 | `(planned) task.buildCockpit` | Task-first cockpit that aggregates readiness, routing, benchmark/regression, trace/session review, provider-run metadata, remediation next steps, and evidence refs by user task; local/read-only by default |
| V2.66 | `(planned) lifecycle.listSkillEvents`, `(planned) lifecycle.summarizeSkillChanges` | Redacted skill lifecycle timeline over scan/provenance, stale/drift, triage, remediation history, prompt analysis, and review evidence; no raw skill content or raw prompt/response persistence |
| V2.67 | `(planned) cleanup.planGuidedFlow`, `(planned) cleanup.recordGuidedStep` | Stepwise guided cleanup over existing findings/similarity/drift/readiness/remediation evidence; any enable/disable/edit still uses existing preview-first explicit-confirm write methods |
