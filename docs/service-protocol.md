...
| V2.58 | `(planned) remediation.previewImpact` | Local-only deterministic impact preview over impacted tasks, agents, skills, risk deltas, snapshot/rollback plan, writable capability/filtering/blockers, and evidence refs before enable/disable/edit/remediation actions; no apply/write/snapshot mutation/triage/script/credential/cloud/telemetry/default-provider side effects, and provider wording stays V2.42 gated copy-only |
