...
## V2.41-V2.43 AI Provider Foundation, Prompt Safety, And Quality Score / V2.44-V2.70 planning

Current implementation status after V2.43:
...
- Implemented in V2.43: `analysis.scoreSkillQuality`; local scoring is deterministic, user-triggered, read-only, and based on catalog/finding/conflict/analysis/adapter diagnostic evidence. Optional provider explanation uses V2.42 prompt preview/redaction/confirmation and remains copy-only. Not implemented yet: V2.44+ task readiness/routing, full provider observability UI, and persistent benchmark/review/policy models.

V2.44-V2.70 should evolve the protocol in this order. Planned method names below are integration-dependent and may change before release if service schema review requires it.
...
| V2.44 | `(integration-dependent) task.checkReadiness` | Task input to agent/skill readiness candidates; read-only result |
| V2.45 | `(integration-dependent) task.rankSkillRoutes` | Candidate ranking, confidence, match reasons, ambiguity and wrong-pick risk |
| V2.46-V2.47 | `(integration-dependent) task.benchmark.*`, `(integration-dependent) task.detectRoutingRegression` | App-local benchmark definitions and deterministic/AI-assisted regression analysis |
| V2.48-V2.49 | `(integration-dependent) trace.importLocal`, `(integration-dependent) trace.analyzeRoutingAccuracy`, `(integration-dependent) routing.dashboard` | Local trace import with redaction; no raw credential/log persistence |
| V2.50-V2.55 | `(integration-dependent) task.compareAgentReadiness`, `(integration-dependent) knowledge.search`, `(integration-dependent) knowledge.groupSimilarSkills`, `(integration-dependent) workspace.checkReadiness` | Local index/readiness views; no default network |
| V2.56-V2.60 | `(integration-dependent) remediation.plan`, `(integration-dependent) remediation.previewDrafts`, `(integration-dependent) remediation.previewImpact`, `(integration-dependent) remediation.history` | AI suggestions are draft/read-only unless user enters existing safe write flow |
| V2.61-V2.68 | `(integration-dependent) reviewSession.*`, `(integration-dependent) policyPack.*`, `(integration-dependent) governance.exportPack` | Local review/policy/governance records and redacted exports |
| V2.69 | `(integration-dependent) llm.listProviderCallMetadata`, `(integration-dependent) llm.summarizeProviderUsage`, `(integration-dependent) llm.clearProviderCallMetadata`, `(integration-dependent) llm.exportProviderUsage` | Full observability UX over V2.41-V2.42 metadata: call history, cost trends, failures, rate limits, availability, cleanup/retention; no secrets/raw prompt/response by default |
| V2.70 | `(integration-dependent) writeEvidence.planExpansion` | Evidence-only safe-write planning; no new writes without verified rollback-safe agent/root support |
...
- Optional provider-backed scoring in a later integration must still route through `llm.previewPrompt` + `llm.confirmPromptAndSend`, with prompt scope, included/excluded summary, redaction status, token/cost estimate, destination preview, and explicit confirmation required.
- Provider-backed scoring cannot write triage/config/snapshot states, mutate toggles, start writes/install, or execute scripts. Draft output remains copy/display only.
- Raw prompt/response remains unserialized by default for quality scoring.
