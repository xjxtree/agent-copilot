...
## V2.41 AI Provider Foundation / V2.42-V2.70 planning
...
- Not implemented yet: V2.42 prompt preview/redaction transport for analysis calls, confirmed provider-backed Analyze/Recommend/skill-analysis calls, full provider observability UI, and persistent benchmark/review/policy models.

V2.42-V2.70 should evolve the protocol in this order. Planned method names below are integration-dependent and may change before release if service schema review requires it.
...
| V2.42 | `(integration-dependent) llm.previewPrompt`, `(integration-dependent) llm.confirmPromptAndSend` | Redaction summary, included/excluded fields, token/cost estimate, destination preview, explicit confirmation before request; confirmed calls record minimal audit metadata |
