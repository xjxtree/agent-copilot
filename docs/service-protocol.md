...
- 批量预览只处理 `agent/roots` 在 adapter matrix 显式支持 config toggle 且当前 session 验证为 verified/guarded 的候选项；Hermes native install-only、OpenClaw 及当前配置受阻实例进入 `skipped` 集合并返回明确 `skip_reason`。Pi 候选项仅在 V2.94 guarded settings scope 内参与批量预览。
- 预览输出应至少包含：
