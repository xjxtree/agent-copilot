...
- Not yet integrated in runtime: V2.57 `remediation.previewDrafts` is planned, while V2.58+ impact/batch/history surfaces plus full V2.69 provider observability UX over call metadata remain future work.

V2.57 planning starts from the completed V2.56 protocol surface below; the planned preview-draft surface stays local-only and copy-only by default.
...
| V2.57 | `(planned) remediation.previewDrafts` | User-triggered, local-only/deterministic-by-default draft suggestions for frontmatter, description, permissions, dependency, and policy; copy/edit-ready only, with no direct apply/write path. Any provider wording must still go through V2.42 prompt preview/redaction/confirmation and remain copy-only |
| V2.58-V2.60 | `(planned) remediation.previewImpact`, `(planned) remediation.history` | Future impact/batch/history work; remains draft/read-only unless user enters an existing safe write flow |
