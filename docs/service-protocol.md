...
| `llm.providerObservability` | No | Integrated (V2.64) | User-triggered deterministic/read-only provider observability over V2.61 prompt run metadata and minimal provider call metadata; returns summary, call/history rows, grouping rows, status rows, budget usage hints, retention recommendations, evidence refs, prompt metadata, and safety flags |
