...
- Source of truth: `crates/service/src/protocol.rs` `SUPPORTED_METHODS`; current count is 93 methods. V2.83 moved this block out of `crates/service/src/lib.rs`; V2.87 added default-off `session.previewLocalSessions` and `evidence.previewMcpServers` without changing existing method semantics, and V2.91 added `llm.listModelTaskMatches`, `llm.recordModelTaskMatch`, and `llm.deleteModelTaskMatch` as app-local model-task history methods.
