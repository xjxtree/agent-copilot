...
| `remediation.batchReview` | No | Planned (V2.59) | Local-only deterministic batch review over task, risk, rule, agent, and workspace groups; review groups/items, safe next-step labels, evidence refs, gap/blocker notes, prompt metadata, and safety flags; no default provider/network/write path |
...
| V2.59 | `(planned) remediation.batchReview` | Planned batch review work; remains local review only unless the user enters an existing safe write flow |
| V2.60 | `(future) remediation.history` | Future remediation history work; remains local review only unless the user enters an existing safe write flow |
...
## V2.59 Batch Review Workflow（planned）

`remediation.batchReview` is the planned local-only, user-triggered, deterministic, read-only-by-default batch review surface for organizing next steps before any existing safe write flow.

- Input shape is expected to cluster review by task, risk, rule, agent, and workspace.
- Planned output should expose summary, review groups, review items, safe next-step labels, gap notes, blocker notes, evidence references, prompt request metadata, and safety flags.
- The planned method must not apply actions, mutate triage, create or roll back snapshots, write skill files or agent config, execute scripts, read credentials, persist raw prompt/response/trace, sync cloud, emit telemetry, or send default provider traffic. Any provider wording, if ever added later, must stay V2.42 gated and copy-only.
