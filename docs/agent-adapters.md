...
> skills-copilot 支持的 6 个 agent 的适配要点。扫描适配器实现 `AgentAdapter`，配置变更适配器实现 `AgentConfigAdapter`，对上层（scanner / catalog / UI）保持透明。
...
> **实现要求**：所有适配器**无状态**。文件内容由 scanner 缓存，`AgentAdapter` 只负责"给出根、解析路径、读取启用状态"；`AgentConfigAdapter` 只封装 agent 特有的配置变更语义，对内存中的 `AgentConfigDocument` 打 patch。实际原子写、snapshot、锁与回滚由 commands/service 层统一执行。这样 listen-then-rescan 的循环里适配器永远不需要持有锁。
...
| --- | --- |
| AgentId | `codex` |
| 状态 | **V2 first implementation complete** —— 第一版可读 verified roots，并只写用户 Codex config；项目级写入启停仍 blocked。adapter core、commands/service、cwd→repo-root discovery 和 macOS UI scan-all 已集成并通过 automated validation；真实本机 Computer Use 操作验证本轮用户豁免 |
...
| Skill 格式 | 官方 Codex docs：skill 是目录，包含必需 `SKILL.md`，frontmatter 里 `name` 和 `description` 必填；缺任一字段应作为 malformed/broken skill 处理，不应让整次 scan 失败；可选 `scripts/`、`references/`、`assets/`、`agents/openai.yaml` 只保留/展示为原始资源，不推导权限 |
...
| 启用控制 | 禁用：用户 config 中为绝对 `SKILL.md` path 写 `enabled = false`；启用：删除用户 config 中该 path 的所有 entries，回到默认发现。重复 entries 本地表现为后者生效，所以 patch 必须先归一化同 path entries。项目 `.codex/config.toml` 禁用在 trusted repo 测试中仍未生效 |
...
Codex 首版可以用用户 config 禁用 user skill 或 project skill，但写入目标仍只能是用户 config。不要写 `<repo>/.codex/config.toml`，不要删除 skill 文件，也不要添加 `enabled = true` 作为 re-enable；re-enable 是删除同一绝对 `SKILL.md` path 的所有 `[[skills.config]]` entries。
...
| --- | --- |
| AgentId | `hermes` |
| 状态 | **Blocked / service evidence only** —— 本机 `hermes-ops` skill 只证明 Hermes 被描述为 macmini 上的服务运维对象，含 CLI、home、repo、日志和 cron 配置线索；没有证明 Hermes 有可扫描的本地 skill 目录或 `SkillInstance` 映射单元 |
...
| --- | --- |
| AgentId | `openclaw` |
| 状态 | **Blocked / partial read-only evidence** —— 本机 OpenClaw 相关 skill docs 给出候选 skill roots、`SKILL.md` 目录输入、`openclaw skills list --eligible`、`openclaw config file` 和 `openclaw.json` plugin 字段线索；这些仍不是维护者确认的 adapter spec |
...
| Skill roots | 官方 opencode Agent Skills docs：项目 `.opencode/skills/<name>/SKILL.md`、`.claude/skills/<name>/SKILL.md`、`.agents/skills/<name>/SKILL.md`；全局 `~/.config/opencode/skills/<name>/SKILL.md`、`~/.claude/skills/<name>/SKILL.md`、`~/.agents/skills/<name>/SKILL.md`；项目 discovery 从 cwd 向上到 Git worktree |
