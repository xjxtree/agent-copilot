...
**当前阶段**：V2.69 Privacy / Screenshot Mode + 本地化收束已完成。原生 UI 默认开启截图隐私模式，对本机路径使用 `$HOME`、`<project-root>`、`<app-data-dir>`、`<temp>` 等占位符和长路径折叠，并提供显式 reveal；Task Cockpit / Guided Cleanup / Provider Observability 的首屏和运行态文案补齐中英文资源；截图 capture helper 会拒绝锁屏、近黑和近单色截图证据。V2.69 只改 UI 展示、app-local 偏好、验证脚本和文档，不改 scanner/catalog 事实层，不新增 provider 默认调用、hidden task state、skill/config write path、triage mutation、snapshot、script execution、cloud sync 或 telemetry。

**近期主线**：V2.70-V2.72 继续 **post-V2.67 consolidation**。事实层仍由本地 scanner/rules/catalog 提供；重点转向 Swift/Rust feature module 拆分、Guided Cleanup 到既有安全动作的 deep links，以及锁屏/黑屏截图验证矩阵加固。短期不做全平台 UI 适配、正式签名 release、notarization、DMG/ZIP 或 public distribution。OpenClaw/Hermes writable/install 与 Pi install 仍保持 blocked；Pi production toggle 仅限 V2.37 evidence-backed guarded native scope，不自动开放兼容根写入。
...
| V2.65-V2.69 | Cockpit / lifecycle / guided cleanup / IA / privacy | 已完成：V2.65 Task-first Cockpit、V2.66 Skill Lifecycle Timeline、V2.67 Guided Cleanup Flow、V2.68 Task Cockpit primary entry / Analysis IA 重组、V2.69 Privacy / Screenshot Mode + 本地化收束 |
...
- 不把 V2.70-V2.72 规划文档视为已实现能力；对应版本必须完成代码、验证和文档 closeout 后才能标记 completed。
