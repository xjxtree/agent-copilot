...
**当前阶段**：V2.72 Validation harness hardening 已完成。验证链路现在有统一 blocker taxonomy、可调用的 `pnpm classify:validation-blocker`、`pnpm verify:validation-blockers`、锁屏 preflight、截图 artifact 黑屏/透明/单色拒绝，以及 fixture/real screenshot 证据矩阵文档。V2.72 不改变产品协议、scanner/catalog 事实层、provider 语义或任何写入/执行路径。
...
- 2026-06-15 real-local audit continuation：current `dist/SkillsCopilot.app` rebuilt and launched. Initial locked-session attempts were correctly rejected as `locked-session` / `window-not-found`; later unlocked Computer Use plus app-window screenshots exercised Task Cockpit, Provider Observability, Local Skill Map, Guided Cleanup safe links, and Review. Task Cockpit can remain in `Preparing...` because task/remediation subcalls do not return within practical UI time; Guided Cleanup safe links route to existing read-only surfaces; Provider Observability returns redacted app-local metadata; Local Skill Map can render real nodes/edges after loading. New follow-ups cover privacy redaction gaps, localization gaps, inherited detail scroll, and visual density; see [`docs/real-local-app-audit-2026-06-15.md`](./docs/real-local-app-audit-2026-06-15.md). V2.73-V2.79 are planned follow-ups, not completed features.
...
| V2.73 | Task / remediation performance and timeout recovery | 规划：优化真实 catalog 下 `task.checkReadiness` / routing / remediation 聚合耗时；为 Task Cockpit 增加超时、失败恢复、取消/重试和可诊断阶段状态 |
...
| V2.78 | Evidence-surface privacy and localization sweep | 规划：统一 Guided Cleanup、Skill Map、Review、Cockpit 等证据面板的路径脱敏/折叠/reveal 行为与中文本地化，确保真实截图不会暴露本地路径 |
| V2.79 | Detail navigation and visual density polish | 规划：切换 Work surface 时重置/恢复合理滚动位置，优化密集证据卡片、长列表、两列布局和阶段摘要，使真实 catalog 下的视觉层级更稳定 |
