...
**当前阶段**：V2.79 Privacy fixture and evidence-surface localization sweep 已完成。隐私检查会拦截固定本地主机端口指纹，Provider fixture 已移除 literal loopback host-port；Task Cockpit、Guided Cleanup、Local Skill Map、Review、Provider Observability、Validation Workbench 与嵌套 evidence cards 统一走路径脱敏/折叠/reveal 与中文本地化。V2.79 不引入 credential reads、network behavior change、scanner/catalog fact mutation、provider 默认调用、写入、执行、cloud sync 或 telemetry。

**V2.78 validation**：multi-agent V2.78 gate parity implementation completed; `pnpm verify:service-protocol-drift`, `pnpm verify:v2.78-docs`, `pnpm verify:gate-parity`, focused service protocol tests, Swift tests, `pnpm check:macos`, `pnpm check:privacy`, and `git diff --check` passed. V2.80-V2.83 remain planned.
...
**下一版本线**：V2.80 Detail navigation and visual density polish；V2.81 Swift service IPC cancellation cleanup；V2.82 provider-test env isolation + core model tests；V2.83 continued module splitting。2026-06-15 Minimax-m3 / GLM-5.1 review findings are assigned into V2.80-V2.83.
...
| `pnpm verify:gate-parity` | Gate parity lane：把 protocol drift、V2.73-V2.79 docs verifiers、validation blocker verifier 和 screenshot artifact verifier 串成 CI/local 共享的 deterministic gate。 |
