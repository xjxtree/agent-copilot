...
**当前阶段**：V2.71 Guided Cleanup safe-action deep links 已完成。`cleanup.planGuidedFlow` 现在为 flow steps 暴露 `safe_action_deep_link`，为 safe next actions 暴露 `deep_link`；Swift UI 会把这些链接显示为安全入口按钮，并只路由到既有 read-only / preview-first surfaces（Findings/Cleanup、Analysis remediation panels、Skill Lifecycle、Task Cockpit、Safe Batch Preview panel、Guided Cleanup metadata record）。V2.71 不新增 apply/write/toggle/script/provider-confirm 路径；safe batch link 只打开 preview 面板上下文，不自动执行 preview 或 apply。

**近期主线**：V2.72 继续 **post-V2.67 consolidation**。事实层仍由本地 scanner/rules/catalog 提供；重点转向锁屏/黑屏截图验证矩阵、fixture/real 截图证据口径和 Computer Use blocker 分类加固。短期不做全平台 UI 适配、正式签名 release、notarization、DMG/ZIP 或 public distribution。OpenClaw/Hermes writable/install 与 Pi install 仍保持 blocked；Pi production toggle 仅限 V2.37 evidence-backed guarded native scope，不自动开放兼容根写入。
...
| V2.65-V2.71 | Cockpit / lifecycle / guided cleanup / IA / privacy / modularization / safe links | 已完成：V2.65 Task-first Cockpit、V2.66 Skill Lifecycle Timeline、V2.67 Guided Cleanup Flow、V2.68 Task Cockpit primary entry / Analysis IA 重组、V2.69 Privacy / Screenshot Mode + 本地化收束、V2.70 Swift / Rust feature modularization、V2.71 Guided Cleanup safe-action deep links |
...
- 不把 V2.72 规划文档视为已实现能力；对应版本必须完成代码、验证和文档 closeout 后才能标记 completed。
