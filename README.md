...
**当前阶段**：V2.75 Task input and input-method resilience 已完成。Task Cockpit 任务输入现在使用稳定 AX-settable 多行输入入口，能保留中文、emoji、换行和非空任务的原始前后空格；空白任务不会提交，任务生成仍只由显式 Build / Generate Cockpit 触发。V2.75 不引入 raw prompt persistence、cloud sync、provider 默认调用、写入、执行、凭据、telemetry、scanner/catalog 事实层或新分析端点语义。
