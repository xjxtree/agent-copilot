...
**当前阶段**：V2.63 Local Skill Map complete。V2.63 新增 `knowledge.buildLocalSkillMap`，把 existing catalog evidence、local knowledge tags、similar groups、capability taxonomy、conflicts、task coverage、readiness/routing/session-review context 与 risk evidence 组织成本地 skill map。该路径是用户触发、deterministic、read-only，不创建新的 source of truth，默认不持久化 map artifact，不保存 raw prompt/raw response/raw trace/secrets/unredacted paths，不写 skill/config、不改 triage、不创建或回滚 snapshot、不执行脚本、不默认发 provider request、不云同步、不发 telemetry。
...
| V2.64-V2.67 | Observability / cockpit / lifecycle / guided cleanup | Planned：V2.64 完善 provider 调用可观测性；V2.65 汇总 task-first cockpit；V2.66 展示 skill lifecycle timeline；V2.67 做 guided cleanup flow |
...
- **AI-native 分析 gate**：规则引擎和 scanner 默认离线提供事实层；provider-backed explanation 只在用户完成 prompt preview/redaction/confirmation 后发送，输出保持 copy-only。V2.61 起，已确认发送的 AI 分析会保存 redacted prompt run metadata 与 copy-only draft output，用于重启后恢复展示；V2.62 起，Agent Session Skill Review 只保存 app-local redacted review metadata 且不发送 provider requests；V2.63 起，Local Skill Map 只派生本地 read-only map，不创建新的 source of truth 或默认持久化 artifact。三者都不保存 raw transcript、raw prompt、raw response、API key、credential、raw trace 或未脱敏本地路径，也不写 skill/config、不改 triage、不执行脚本、不发 telemetry。
