...
**当前阶段**：V2.86 Rust helper/test split and module-size gate closeout completed. V2.1-V2.86 is the synchronized completed baseline; V2.84-V2.86 complete the post-V2.83 module-splitting line with Swift Detail section splitting, Rust RPC domain module splitting, Rust helper/test split, `verify:module-size`, split-file verifier parity, and unchanged service protocol / native UI semantics.

**V2.78 validation**：multi-agent V2.78 gate parity implementation completed; `pnpm verify:service-protocol-drift`, `pnpm verify:v2.78-docs`, `pnpm verify:gate-parity`, focused service protocol tests, Swift tests, `pnpm check:macos`, `pnpm check:privacy`, and `git diff --check` passed. V2.82 and V2.83 are now completed after V2.78 closeout.
...
**V2.81 validation**：V2.81 Swift service IPC cancellation cleanup completed; multi-agent V2.81 implementation completed; Swift `ServiceClient` delegates stdio sidecar execution to a cancellable runner, Task Cockpit cancel/timeout now cancels the active service task, and focused Swift tests cover normal cancellation plus TERM-ignoring sidecar force-kill cleanup. `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:service-protocol-drift`, `pnpm verify:v2.81-docs`, `pnpm verify:gate-parity`, `pnpm check:macos`, `pnpm check:privacy`, and `git diff --check` passed. No fresh Computer Use screenshot is required because V2.81 does not change user-visible native UI. Boundary preserved: no daemon/socket redesign by default, service protocol method/payload changes, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution, signing/notarization/DMG/ZIP.

**V2.82 validation**：V2.82 test isolation and core model test floor completed; provider tests that mutate environment variables now use serialized RAII cleanup, and `crates/core` has focused tests for stable `AgentId` / `Scope` wire strings, `PermissionRequest` safe defaults, and skill identity/state fields without adding serde dependencies. `cargo test -p skills-copilot-core -- --nocapture`, focused provider isolation tests, `cargo test --workspace`, `pnpm verify:v2.82-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, no-capture fixture smoke, and `git diff --check` passed. `pnpm check:macos` and `./script/build_and_run.sh --verify` failed closed with canonical `locked-session` before UI evidence capture; no fresh Computer Use screenshot is required because V2.82 has no user-visible native UI or service-protocol behavior change. Boundary: no provider credential persistence changes, service protocol method/payload changes, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads beyond existing explicitly confirmed provider tests, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP.

**V2.83 validation**：multi-agent V2.83 implementation completed; Rust service protocol constants/envelopes moved into `crates/service/src/protocol.rs`, Detail overview code moved into `DetailOverviewSection.swift`, fake service test helpers moved into `FakeServiceScript.swift`, and the protocol/native UI verifiers now aggregate split files. `cargo test -p skills-copilot-service supported_methods_have_dispatch_coverage -- --nocapture`, `cargo test -p skills-copilot-service service_protocol_fixtures_decode -- --nocapture`, `pnpm verify:service-protocol-drift`, `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:v2.83-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, no-capture fixture smoke, and `git diff --check` passed. `pnpm check:macos` failed closed with canonical `locked-session` before UI evidence capture; no fresh Computer Use screenshot is required because V2.83 has no user-visible native UI or service-protocol behavior change. Boundary: no service protocol method or payload changes, protocol version bump, new UI surface, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP.

**V2.84 validation**：V2.84 Swift Detail section splitting completed; `DetailView.swift` is now a small routing/composition file while `DetailGuidedCleanupFlowPanel.swift`, `DetailProviderObservabilityPanel.swift`, `DetailReviewCoreSection.swift`, and the other split Detail section files hold the existing Task Cockpit / Skill Map / Guided Cleanup / Provider Observability / Review UI. `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:module-size`, `pnpm verify:v2.84-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, and `git diff --check` passed. No fresh Computer Use screenshot is required because V2.84 has no user-visible native UI behavior change.

**V2.85 validation**：V2.85 Rust RPC domain module splitting completed; `ServiceHost` implementation moved into `service_host.rs`, `service_cleanup.rs`, `service_knowledge.rs`, `service_llm.rs`, `service_remediation.rs`, and `service_task.rs` while `crates/service/src/lib.rs` remains the small crate-root DTO/protocol surface. `pnpm verify:service-protocol-drift` now scans the split service implementation files, and focused dispatch/protocol fixture tests plus `pnpm verify:module-size`, `pnpm verify:v2.85-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, and `git diff --check` passed. Boundary: no service protocol method or payload changes, protocol version bump, provider default calls, write/apply paths, script execution, credential reads, cloud sync, telemetry, or public distribution/signing/notarization/DMG/ZIP.

**V2.86 validation**：V2.86 Rust helper/test split and module-size gate closeout completed; service helpers moved into `service_support_helpers.rs`, `service_knowledge_helpers.rs`, `service_remediation_helpers.rs`, `service_task_helpers.rs`, `service_observability_helpers.rs`, `service_llm_prompt_helpers.rs`, and `service_guided_cleanup_helpers.rs`, and Rust service tests moved into `crates/service/src/tests.rs` plus focused chunks under `crates/service/src/tests/`. `pnpm verify:module-size` verifies the split Rust service files, Rust test chunks, and Swift Detail files are <= 5000 lines and is wired into `pnpm verify:gate-parity`. Full Rust workspace tests, clippy, Swift tests, protocol drift, native UI layout, V2.84-V2.86 docs verifiers, gate parity, privacy, diff checks, and `pnpm check:macos` passed.

**下一版本线**：V2.84-V2.86 module-splitting closeout completed. No post-V2.86 version has been selected yet.
...
- 2026-06-15 V2.77 validation：multi-agent V2.77 implementation completed; the native Work surface now exposes a read-only Validation Workbench / 验证工作台 with `skills-copilot.validation-workbench`, canonical blocker rows, evidence standards, fixture-smoke supporting-only guidance, and no runnable validation actions. Focused Swift model tests, `pnpm verify:macos-ui-layout`, `pnpm check:macos`, `pnpm check:privacy`, screenshot artifact verification, `pnpm verify:v2.77-docs`, and `git diff --check` passed. Unlocked Computer Use targeted `dist/SkillsCopilot.app`, resolved PID `34909`, verified `skills-copilot.validation-workbench`, `skills-copilot.validation-workbench.summary`, `skills-copilot.validation-workbench.evidence-standards`, canonical blocker rows, and captured evidence at [`docs/ui-artifacts/v2.77-validation-workbench/completed.png`](./docs/ui-artifacts/v2.77-validation-workbench/completed.png). V2.78-V2.83 remained follow-ups after V2.77 closeout.
- 2026-06-15 V2.78 validation：multi-agent V2.78 implementation completed; service protocol docs now cover all 88 `SUPPORTED_METHODS`, file-level session review fixtures were added, `service.status.supported_methods` fixture is drift-checked, `pnpm verify:service-protocol-drift` and `pnpm verify:gate-parity` are wired into local/CI gates, and V2.46-V2.64 verification-history governance is documented without inventing missing evidence. `pnpm check:macos`, `pnpm check:privacy`, focused protocol tests, Swift tests, `pnpm verify:v2.78-docs`, and `git diff --check` passed. No fresh Computer Use screenshot is required for V2.78 because no user-visible native UI surface changed.
- 2026-06-15 V2.79 validation：multi-agent V2.79 implementation completed; privacy scanning now catches non-allowlisted fixed loopback host-port fingerprints, local provider fixtures use non-local fixture destinations, and evidence surfaces use screenshot-safe path rendering plus English/zh-Hans coverage. `pnpm verify:v2.79-docs`, `pnpm verify:gate-parity`, focused privacy/localization checks, Swift tests, `pnpm check:macos`, `pnpm check:privacy`, screenshot artifact verification, and `git diff --check` passed. Unlocked Computer Use targeted `dist/SkillsCopilot.app`, resolved PID `68064`, verified localized Work surfaces and `skills-copilot.validation-workbench`, and captured evidence at [`docs/ui-artifacts/v2.79-privacy-localization/completed.png`](./docs/ui-artifacts/v2.79-privacy-localization/completed.png).
- 2026-06-15 V2.80 validation：multi-agent V2.80 implementation completed; Detail navigation resets to a stable top anchor when switching major surfaces, and dense evidence/safety lists are counted, collapsible, and screenshot-safe. `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:v2.80-docs`, `pnpm verify:gate-parity`, `pnpm check:macos`, `pnpm check:privacy`, screenshot artifact verification, and `git diff --check` passed. Unlocked Computer Use targeted `dist/SkillsCopilot.app`, resolved PID `82571`, verified `skills-copilot.validation-workbench` after switching from a scrolled `复查` surface, and captured evidence at [`docs/ui-artifacts/v2.80-detail-density/completed.png`](./docs/ui-artifacts/v2.80-detail-density/completed.png).
- 2026-06-15 V2.81 validation：multi-agent V2.81 implementation completed; Swift stdio sidecar calls now use cancellable process cleanup with bounded SIGKILL escalation, pipe handle cleanup, and Task Cockpit service-task cancellation. `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:service-protocol-drift`, `pnpm verify:v2.81-docs`, `pnpm verify:gate-parity`, `pnpm check:macos`, `pnpm check:privacy`, and `git diff --check` passed. No fresh Computer Use screenshot is required because no user-visible native UI changed.
- 2026-06-15 V2.82 validation：multi-agent V2.82 implementation completed; provider environment mutation tests now use serialized RAII cleanup, and core model tests lock wire/default/identity stability without adding serde dependencies. `cargo test -p skills-copilot-core -- --nocapture`, focused provider isolation tests, `cargo test --workspace`, `pnpm verify:v2.82-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, no-capture fixture smoke, and `git diff --check` passed. `pnpm check:macos` failed closed at fixture capture with canonical `locked-session`, and `./script/build_and_run.sh --verify` hit the same blocker before UI evidence; no fresh Computer Use screenshot is required because no user-visible native UI or service-protocol behavior changed.
- 2026-06-15 V2.83 validation：multi-agent V2.83 implementation completed; Rust protocol DTO/constants, Swift Detail overview UI helpers, and Swift fake service test helpers were split into dedicated files while verifiers were updated to read those split modules. Focused Rust protocol tests, `pnpm verify:service-protocol-drift`, `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:v2.83-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, no-capture fixture smoke, and `git diff --check` passed. `pnpm check:macos` failed closed with canonical `locked-session`; no fresh Computer Use screenshot is required because no user-visible native UI or service-protocol behavior changed.
- 2026-06-16 V2.84 validation：Swift Detail section splitting completed; `DetailView.swift` became a router/composer and split section files such as `DetailGuidedCleanupFlowPanel.swift`, `DetailProviderObservabilityPanel.swift`, and `DetailReviewCoreSection.swift` preserve existing UI semantics. `swift test --package-path apps/macos`, `pnpm verify:macos-ui-layout`, `pnpm verify:module-size`, `pnpm verify:v2.84-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, and `git diff --check` passed.
- 2026-06-16 V2.85 validation：Rust RPC domain module splitting completed; `ServiceHost` moved into `service_host.rs`, `service_cleanup.rs`, `service_knowledge.rs`, `service_llm.rs`, `service_remediation.rs`, and `service_task.rs`; `pnpm verify:service-protocol-drift` scans split service files and still reports 88 documented/supported/dispatch methods. Focused protocol tests, `pnpm verify:module-size`, `pnpm verify:v2.85-docs`, `pnpm verify:gate-parity`, `pnpm check:privacy`, and `git diff --check` passed.
- 2026-06-16 V2.86 validation：Rust helper/test split completed; helper include files such as `service_support_helpers.rs` and service test chunks under `crates/service/src/tests/` are guarded by `pnpm verify:module-size`. Full Rust workspace tests, clippy, Swift tests, protocol drift, native UI layout, docs verifiers, gate parity, privacy, diff checks, and `pnpm check:macos` passed.
...
| V2.82 | Test isolation and core model test floor | 已完成：provider env mutation tests use serialized RAII cleanup; core model tests lock `AgentId` / `Scope` wire strings, safe defaults, and identity/state fields without adding serde dependencies |
| V2.83 | Continued module splitting | 已完成：拆分 Rust service protocol DTO/constants、Swift Detail overview helpers 和 Swift fake service test helpers；更新 split-file verifier parity；不改变 service protocol、UI labels/navigation 或产品语义 |
| V2.84 | Swift Detail section splitting | 已完成：`DetailView.swift` 作为 router/composer，Task Cockpit / Skill Map / Guided Cleanup / Provider Observability / Review 相关 section view 拆入 `Detail*.swift` 文件；`verify-native-ui-layout` 聚合 split Detail surface，`verify:module-size` 约束 <= 5000 行 |
| V2.85 | Rust RPC domain module splitting | 已完成：`ServiceHost` RPC 实现按 domain 拆到 `service_host.rs`、`service_cleanup.rs`、`service_knowledge.rs`、`service_llm.rs`、`service_remediation.rs`、`service_task.rs`；protocol drift verifier 扫描 split service files |
| V2.86 | Rust helper/test split and module-size gate | 已完成：service helper include files 与 `crates/service/src/tests/` 测试 chunks 均拆分到 <= 5000 行；`pnpm verify:module-size` 接入 `pnpm verify:gate-parity` |
...
| V2.82 验证清单（completed） | [`docs/v2.82-verification-checklist.md`](./docs/v2.82-verification-checklist.md) |
...
| `pnpm verify:service-protocol-drift` | Service protocol drift verifier：检查 `SUPPORTED_METHODS`、split `handle_result` dispatch、`docs/service-protocol.md` method table、`service.status.supported_methods` fixture 与 request/response fixture cases 一致。 |
| `pnpm verify:module-size` | V2.86 module-size gate：验证 split Rust service files、`crates/service/src/tests/` chunks、Swift Detail section files、`TaskCockpitPanel.swift` 和 `ValidationWorkbenchPanel.swift` 均不超过 5000 行。 |
| `pnpm verify:gate-parity` | Gate parity lane：把 protocol drift、module-size、V2.73-V2.86 docs verifiers、validation blocker verifier 和 screenshot artifact verifier 串成 CI/local 共享的 deterministic gate。 |
...
| `pnpm verify:v2.82-docs` | 验证 V2.82 completed closeout 覆盖 provider env mutation RAII isolation、core model wire/default/identity test floor、shared gate evidence、no fresh UI screenshot required decision，以及 no provider credential persistence/service protocol/provider default/write/hidden-state/scanner/script/raw-persistence/cloud/telemetry/public-distribution expansion。 |
| `pnpm verify:v2.83-docs` | 验证 V2.83 验证清单（completed）覆盖 continued module splitting、split protocol/detail/test fixture modules、shared gate evidence、no fresh UI screenshot required decision，以及 no service protocol method or payload changes, protocol version bump, new UI surface, provider default calls, write/apply paths, hidden task state, scanner/catalog fact mutation, script execution, credential reads, raw prompt/response/trace persistence, cloud sync, telemetry, public distribution/signing/notarization/DMG/ZIP。 |
| `pnpm verify:v2.84-docs` | 验证 V2.84 Swift Detail section splitting completed closeout，覆盖 `DetailView.swift` router/composer、split `Detail*.swift` section files、`verify-native-ui-layout` 聚合 split surface、`verify:module-size` 和 no fresh UI evidence required decision。 |
| `pnpm verify:v2.85-docs` | 验证 V2.85 Rust RPC domain module splitting completed closeout，覆盖 `service_host.rs` / `service_task.rs` 等 split service files、protocol drift verifier 多文件扫描、88-method protocol parity 和 refactor-only safety boundary。 |
| `pnpm verify:v2.86-docs` | 验证 V2.86 Rust helper/test split and module-size gate completed closeout，覆盖 `service_support_helpers.rs` 等 helper files、`crates/service/src/tests/` chunks、`verify:module-size` 接入 gate parity 和 full validation evidence。 |
