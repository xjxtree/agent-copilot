...
- 2026-06-15 real-local audit continuation：current `dist/SkillsCopilot.app` rebuilt and launched, but the macOS session is locked (`CGSSessionScreenIsLocked=1`), Computer Use returns `cgWindowNotFound`, and direct capture fails closed as `locked-session`; see [`docs/real-local-app-audit-2026-06-15.md`](./docs/real-local-app-audit-2026-06-15.md). Before lock, Task Cockpit first-screen observation succeeded; real data testing found `task.buildCockpit` can remain in `Preparing...` because task/remediation service subcalls do not return within practical UI time. V2.73-V2.77 are planned follow-ups, not completed features.
