...
**当前阶段**：V2.70 Swift / Rust feature modularization 已完成。`DetailView.swift` 已拆出 `TaskCockpitPanel.swift` 与 `DetailPresentationPrimitives.swift`，Rust service 已拆出 read-only `cleanup_queue.rs` feature module；`scripts/verify-native-ui-layout.mjs` 已改为聚合拆分后的视图文件继续验证 Task Cockpit / 隐私路径 / IA 约束。V2.70 只调整模块边界，不改变 JSON-RPC method、payload、scanner/catalog 事实层、provider 默认调用、write path、triage、snapshot、script execution、cloud sync 或 telemetry。

**近期主线**：V2.71-V2.72 继续 **post-V2.67 consolidation**。事实层仍由本地 scanner/rules/catalog 提供；重点转向 Guided Cleanup 到既有安全动作的 deep links，以及锁屏/黑屏截图验证矩阵加固。短期不做全平台 UI 适配、正式签名 release、notarization、DMG/ZIP 或 public distribution。OpenClaw/Hermes writable/install 与 Pi install 仍保持 blocked；Pi production toggle 仅限 V2.37 evidence-backed guarded native scope，不自动开放兼容根写入。
...
| V2.65-V2.70 | Cockpit / lifecycle / guided cleanup / IA / privacy / modularization | 已完成：V2.65 Task-first Cockpit、V2.66 Skill Lifecycle Timeline、V2.67 Guided Cleanup Flow、V2.68 Task Cockpit primary entry / Analysis IA 重组、V2.69 Privacy / Screenshot Mode + 本地化收束、V2.70 Swift / Rust feature modularization |
...
- 不把 V2.71-V2.72 规划文档视为已实现能力；对应版本必须完成代码、验证和文档 closeout 后才能标记 completed。
