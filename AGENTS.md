...
- Near-term priority: keep the product focused on skills management, inspection, analysis, and configuration audit. Next work should prioritize V2.38 Hermes external roots while preserving V2.27 provenance/identity explainability, V2.28 conflict semantics, V2.29 finding triage persistence, V2.30 read-only AI skill analysis workflow, V2.31 Cleanup Queue, V2.32 local rule tuning/suppression, V2.33 safe batch actions, V2.34 cross-agent comparison, V2.35 local report export, V2.36 Pi writable evidence harness, and V2.37 Pi guarded toggle. Do not reintroduce full-platform UI adaptation, formal signed release/notarization/DMG/ZIP/public distribution, script execution, cloud sync, telemetry, or automatic write paths.
...
- V2.38 文档与界面/协议同步后的最小验收预期：focused checks（必要时附加契约测试）、`pnpm check:macos`、`pnpm check:privacy`，以及明确针对当前 `dist/SkillsCopilot.app` 的 real local macOS Computer Use 验证；若 Computer Use 工具再次报 `cgWindowNotFound`，必须记录直接 CG/AX 与 smoke 证据并保留 blocker。
