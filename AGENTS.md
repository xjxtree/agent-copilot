...
- Current phase: **V2.23 文档与验收口径同步（进行中）**。V2.22 finding/conflict 语义与验收对齐作为前置约束：conflict = same-agent runtime/name collision；cross-agent duplicate/source overlap is analysis insight。Pi writable support remains an evidence-harness candidate; writable/install support remains blocked until rollback-safe evidence passes.
- Near-term priority: V2.23 Health Dashboard / Adapter Capability UX 的 selected-agent 侧栏口径、health 卡片行动摘要口径、finding/conflict 计数与 issue group 对齐，并保持 Pi writable evidence、finding triage persistence、agent-config timeline 为候选后续。
