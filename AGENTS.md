...
- V2.47 planned boundary: routing regression detection will compare benchmark outputs against baselines using app-local evidence only; no provider request may participate in local regression scoring.
...
- Current mainline real local app validation status: V2.46 passes focused Rust/Swift checks and `pnpm check:macos`; real local app launch against the current `dist/SkillsCopilot.app` bundle succeeded and direct window capture found the app window. Computer Use is currently blocked for the current bundle: absolute bundle path returns `cgWindowNotFound`, and app-name lookup returns `timeoutReached` even after stale same-bundle-id worktree processes were removed. The captured real-local screenshot exposed local paths and was not committed. Keep this tool-layer blocker explicit for subsequent UI/service/protocol changes; do not substitute fixture smoke screenshots for real local Computer Use validation.
...
- Current implementation gap after V2.46: code now has provider profile configuration, Keychain-first API key storage, explicit `llm.testProviderConnection`, V2.42 `llm.previewPrompt` / `llm.confirmPromptAndSend`, redacted prompt previews, token/cost estimate, destination preview, confirmation-gated provider send, minimal redacted call metadata, V2.43 deterministic skill quality scoring, V2.44 deterministic task readiness, V2.45 deterministic routing confidence via `task.rankSkillRoutes`, and V2.46 app-local task benchmark CRUD/evaluation via `task.listBenchmarks` / `task.saveBenchmark` / `task.deleteBenchmark` / `task.evaluateBenchmarks`. No V2.47+ regression or trace analysis yet, and no full V2.69 provider observability UI。
...
- V2.7/V2.30 LLM local assist remains disabled by default until the user configures a provider. V2.41 adds provider profile settings, Keychain-first credentials, and explicit Test Connection; V2.42 adds prompt preview/redaction and confirmed provider-backed draft output; V2.43 adds deterministic read-only skill quality scoring; V2.44 adds deterministic read-only task readiness; V2.45 adds deterministic read-only routing confidence; V2.46 adds deterministic app-local task benchmark evaluation. Do not claim V2.47+ regression/trace capabilities exist until those paths are implemented and validated. All future provider calls must keep explicit opt-in, prompt preview/redaction, budget/observability, and no-write/no-execute guarantees.
