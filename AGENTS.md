...
- Current implementation gap after V2.41: code now has provider profile configuration, Keychain-first API key storage, explicit `llm.testProviderConnection`, budget fields, and minimal redacted test-call metadata. It still has no V2.42 prompt preview/redaction transport for provider-backed analysis calls, no confirmed Analyze/Recommend/skill-analysis provider execution, no full provider observability UI, and no V2.43+ quality/readiness/routing outputs.
...
- V2.7/V2.30 LLM local assist currently remains disabled by default for analysis calls. V2.41 adds provider profile settings, Keychain-first credentials, and explicit Test Connection only; do not claim provider-backed analysis execution, prompt preview/redaction, or quality/readiness/routing exists until V2.42+ implements and validates those paths. All future provider calls must keep explicit opt-in, prompt preview/redaction where applicable, budget/observability, and no-write/no-execute guarantees.
