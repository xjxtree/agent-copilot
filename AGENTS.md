...
- Current phase: V2.16-V2.20 skills management and analysis line is complete. V2.21 scan accuracy, dedupe, and agent-dimension statistics sync is complete. V2.22 finding dedupe and conflict semantics are complete. OpenClaw and Hermes read-only scanners, cross-agent analysis, skill health dashboard, and read-only AI skill analysis assist are integrated. **V2.22 finding/conflict 语义与验收同步进行中**（conflict = same-agent runtime/name collision；cross-agent duplicate/source overlap is analysis insight）。Pi writable support remains an evidence-harness candidate; writable/install support remains blocked until rollback-safe evidence passes.
