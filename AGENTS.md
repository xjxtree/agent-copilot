...
- Current phase: **V2.25 Agent-config timeline 规划与验收同步（进行中）**。V2.22 finding/conflict 语义与验收对齐已收口；V2.23 Health Dashboard / Adapter Capability UX 与 V2.24 Detail 单 skill 口径继续稳定对齐。当前聚焦：仅交付 agent-config timeline，按 agent 独立记录事件，不与 selected skill detail 混用；History 仍仅 toggle/config 事件；不新增 skill-content snapshot；不新增 skill-toggle snapshot。
- Near-term priority: V2.25 Agent-config timeline 口径同步：统一按 agent 的配置快照时间线；`snapshot.previewRollback` 与 `snapshot.rollback` 均需二次确认；沿用 V2.24 Detail 的 findings/conflicts/analysis/read-only offline 与 history=toggle/config 边界；V2.23 selected-agent 侧栏、行动摘要、adapter matrix 仍为基础验收约束。
