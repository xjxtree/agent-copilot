...
- V2.33 Safe batch actions 边界：只面向已 verified writable 的 agent/roots 提供批量 enable/disable 的 preview-first 工作流；必须显示受影响 skill、adapter capability、snapshot/rollback 计划和不可写过滤结果；Pi/Hermes/OpenClaw 保持 read-only；不新增脚本执行、AI provider 调用、credential 存储或 public distribution 任务。
