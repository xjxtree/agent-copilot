...
- Current phase: **V2.24 Skill Detail 诊断工作台规划与验收同步（进行中）**。V2.22 finding/conflict 语义与验收对齐仍作为前置约束；V2.23 Health Dashboard / Adapter Capability UX 口径作为基础保持同步；当前聚焦 Detail 单 skill 诊断口径，Findings = issue groups，Conflicts = selected/current agent same-agent runtime/name collision；Analysis = read-only/offline assist；History = toggle/config 事件。Pi writable support remains an evidence-harness candidate; writable/install support remains blocked until rollback-safe evidence passes.
- Near-term priority: V2.24 Skill Detail 诊断工作台口径同步：Detail 为单 skill 诊断工作台；Findings 与 catalog issue groups 对齐；Conflicts 仅 current-agent runtime/name collision；Analysis 保持 read-only/offline；History 仅 toggle/config 时间线。V2.23 口径（selected-agent 侧栏、健康卡片行动摘要、adapter matrix）应稳定后再进入 V2.24 收敛。
...
- Completed management/analysis line: V2.16 OpenClaw read-only scanner, V2.17 Hermes read-only scanner, V2.18 cross-agent skill analysis, V2.19 skill health dashboard and triage UX, V2.20 read-only AI skill analysis assist.
...
- Near-term work prioritizes scoped adapter implementation from evidence: OpenClaw read-only scanner, Hermes read-only scanner, then Pi writable evidence harness. Pi direct root `.md` cataloging is intentionally disabled after real local validation showed it pulls ordinary resource documents into the skills list. Do not reopen V2.1-V2.15 for those closed items.
