...
- Current implementation gap after V2.47: code now has provider profile configuration, Keychain-first API key storage, explicit `llm.testProviderConnection`, V2.42 `llm.previewPrompt` / `llm.confirmPromptAndSend`, redacted prompt previews, token/cost estimate, destination preview, confirmation-gated provider send, minimal redacted call metadata, V2.43 deterministic skill quality scoring, V2.44 deterministic task readiness, V2.45 deterministic routing confidence via `task.rankSkillRoutes`, V2.46 app-local task benchmark CRUD/evaluation via `task.listBenchmarks` / `task.saveBenchmark` / `task.deleteBenchmark` / `task.evaluateBenchmarks`, and V2.47 routing regression baseline/detection via `task.saveRoutingBaseline` / `task.detectRoutingRegression`. No V2.48+ trace analysis yet, and no full V2.69 provider observability UI。
...
- V2.7/V2.30 LLM local assist remains disabled by default until the user configures a provider. V2.41 adds provider profile settings, Keychain-first credentials, and explicit Test Connection; V2.42 adds prompt preview/redaction and confirmed provider-backed draft output; V2.43 adds deterministic read-only skill quality scoring; V2.44 adds deterministic read-only task readiness; V2.45 adds deterministic read-only routing confidence; V2.46 adds deterministic app-local task benchmark evaluation; V2.47 adds deterministic app-local routing regression detection. Do not claim V2.48+ trace capabilities exist until those paths are implemented and validated. All future provider calls must keep explicit opt-in, prompt preview/redaction, budget/observability, and no-write/no-execute guarantees.
