...
- Current phase: **V2.29 finding triage persistence active**。V2.21-V2.28 已收口：scan accuracy/dedupe、finding/conflict 语义、Health/Adapter Capability UX、single-skill Detail 诊断口径、Agent-config timeline、Finding explainability、Skill identity/provenance dedupe、Conflict semantic closeout 均已同步到主线。
...
- Near-term priority: keep the product focused on skills management, inspection, analysis, and configuration audit. Next work should prioritize finding triage persistence and read-only AI skill analysis workflow while preserving V2.27 provenance/identity explainability and V2.28 conflict semantics. Do not reintroduce full-platform UI adaptation, formal signed release/notarization/DMG/ZIP/public distribution, script execution, cloud sync, telemetry, or automatic write paths.
- V2.29 persistence 边界：仅 app-local triage state，不写 agent config；继续无 skill-content snapshot、无 skill-toggle snapshot、无脚本执行、无 AI 写入/凭据存储。
