...
- Current implementation gap after V2.45: code now has provider profile configuration, Keychain-first API key storage, explicit `llm.testProviderConnection`, V2.42 `llm.previewPrompt` / `llm.confirmPromptAndSend`, redacted prompt previews, token/cost estimate, destination preview, confirmation-gated provider send, minimal redacted call metadata, V2.43 deterministic skill quality scoring, V2.44 deterministic task readiness, and V2.45 deterministic routing confidence via `task.rankSkillRoutes`. No V2.46+ benchmark/regression or trace analysis yet, and no full V2.69 provider observability UI。
...
- V2.7/V2.30 LLM local assist remains disabled by default until the user configures a provider. V2.41 adds provider profile settings, Keychain-first credentials, and explicit Test Connection; V2.42 adds prompt preview/redaction and confirmed provider-backed draft output; V2.43 adds deterministic read-only skill quality scoring; V2.44 adds deterministic read-only task readiness; V2.45 adds deterministic read-only routing confidence. Do not claim V2.46+ benchmark/regression/trace capabilities exist until those paths are implemented and validated. All future provider calls must keep explicit opt-in, prompt preview/redaction, budget/observability, and no-write/no-execute guarantees.
