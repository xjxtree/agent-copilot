...
- Near-term priority: keep the product focused on skills management, inspection, analysis, and configuration audit. Next work should prioritize V2.40 adapter diagnostics while preserving V2.27 provenance/identity explainability, V2.28 conflict semantics, V2.29 finding triage persistence, V2.30 read-only AI skill analysis workflow, V2.31 Cleanup Queue, V2.32 local rule tuning/suppression, V2.33 safe batch actions, V2.34 cross-agent comparison, V2.35 local report export, V2.36 Pi writable evidence harness, V2.37 Pi guarded toggle, V2.38 Hermes external roots, and V2.39 OpenClaw workspace deepening. Do not reintroduce full-platform UI adaptation, formal signed release/notarization/DMG/ZIP/public distribution, script execution, cloud sync, telemetry, or automatic write paths.
...
- V2.39 验证：focused OpenClaw adapter/scanner/commands tests、list model、Swift tests、`pnpm check:macos` 已通过；真实 app smoke launcher 能启动当前 `dist/SkillsCopilot.app` 并取得窗口 id，但 Computer Use 返回 `cgWindowNotFound`，System Events 看到 0 个窗口，capture 无可见窗口，作为工具/窗口层 blocker 保留；后续 V2.40 UI/service/protocol 变更仍需重跑真实交互验证。
