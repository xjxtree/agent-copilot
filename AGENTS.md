...
- V2.41-V2.70 规划边界：AI 大模型成为复杂分析/判断的核心优势，但事实层仍由本地 deterministic scanner/rules/catalog 提供。Provider 必须由用户显式配置 endpoint / API key / model，优先支持 OpenAI-compatible 与 Claude-compatible 接口；API key 优先 Keychain，不能写入 SQLite、项目目录、日志、报告或 prompt artifact；V2.41-V2.42 必须提供最小 provider call audit metadata，V2.69 再做完整 observability UX；所有 AI 分析必须 user-triggered、prompt-previewed、redacted、read-only by default，不能自动写回、自动执行或自动修改 agent config。
...
- Current implementation gap before V2.41: code already has disabled-by-default `llm.status`, `llm.prepareAction`, `llm.prepareSkillAnalysis`, and native read-only preview UI, but it still has no real provider client, no network call path, no endpoint/key settings flow, and no credential persistence. Do not claim OpenAI-compatible or Claude-compatible provider execution exists until V2.41+ implements and validates it.
...
- V2.7/V2.30 LLM local assist currently remains disabled by default. The finished implementation boundary is service/UI gate plus request prepare/estimate; do not claim real provider clients, network calls, endpoint settings, or credential storage exist. V2.41+ may add real user-configured provider calls only behind explicit opt-in, prompt preview/redaction, Keychain-first credentials, budget/observability, and no-write/no-execute guarantees.
...
- Optional LLM features must be explicitly enabled by the user. V2.7/V2.30 currently must not save credentials; V2.30 read-only analysis remains disabled-by-default local prepare/preview and explicit action-triggered while no provider calls run by default. V2.41+ provider settings must support user-owned OpenAI-compatible and Claude-compatible endpoints without hardcoding a vendor. macOS credential storage must prefer Keychain, and fallback `~/.config/skills-copilot/llm.yaml` must be permission-checked as `0600` and contain only non-secret provider metadata unless the user explicitly accepts fallback secret storage. Credentials must never be written to SQLite, project directories, logs, prompts, response artifacts, screenshots, or reports.
